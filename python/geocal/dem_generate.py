...
class DemGenerateMB(geocal_swig.CalcRasterMultiBand):
    ...
    def calc(self, lstart, sstart):
        ...
        self.data[0, :, :] = self.dem_generate.height_grid(
            interpolate_method=self.interpolate_method, buffer_size=self.buffer_size
        )
...
class DemGenerate(object):
    ...
    def __init__(
        ...
        if surface_image1 is None or surface_image2 is None:
            self.itoim = IgcImageToImageMatch(self.igc1, self.igc2, self.image_matcher)
        else:
            self.itoim = SurfaceImageToImageMatch(
                self.igc1,
                self.surface_image1,
                self.igc2,
                self.surface_image2,
    ...
    def surface_point(self, *arg):
        ...
        if len(arg) == 5:
            ...
            r = self.dem_match.surface_point(
                lstart, sstart, lend, send, self.stride, self.stride, include_image
            )
        elif len(arg) == 2:
        ...
        log.info("  Image masked:                 %d" % (self.dem_match.diagnostic[1]))
        log.info("  Too close to image edge:      %d" % (self.dem_match.diagnostic[2]))
        log.info("  Variance too low:             %d" % (self.dem_match.diagnostic[3]))
        log.info("  Correlation too low:          %d" % (self.dem_match.diagnostic[4]))
        log.info("  Exceed max sigma:             %d" % (self.dem_match.diagnostic[5]))
        log.info("  Exceed max radiance variance: %d" % (self.dem_match.diagnostic[6]))
        log.info("  Exceed precision requirement: %d" % (self.dem_match.diagnostic[7]))
    ...
    def find_intersection(self, ic1, ic2):
        ...
        gres = self.coordinate_converter.convert_from_coordinate(
            p.longitude, p.latitude, p.height_reference_surface
        )
        if dist > 1.0:
    ...
    def write_ply(self, filename, scale_z=1.0):
        ...
        with PlyFile(filename) as fh:
            ...
            for i in range(self.r.shape[0]):
                fh.vertex[i, 1], fh.vertex[i, 0] = self.aoi.coordinate(
                    self.coordinate_converter.convert_from_coordinate(
                        self.r[i, 1], self.r[i, 0]
                    )
                )
        ...
        if isinstance(self.itoim, SurfaceImageToImageMatch):
            ...
            for x in range(0, numx, xstep):
                for y in range(0, numy, ystep):
                    arg.append([self.aoi.subset(x, y, xstep, ystep), include_image])
        else:
            if lstart > sstart:
    ...
    def height_grid(
        ...
        self.h = np.empty((self.aoi.number_y_pixel, self.aoi.number_x_pixel))
        self.h[:] = fill_value
        if include_image:
            self.img1_h = np.empty((self.igc1.number_line, self.igc1.number_sample))
            self.img2_h = np.empty((self.igc2.number_line, self.igc2.number_sample))
            self.img1_h[:] = fill_value
            self.img2_h[:] = fill_value
        for i in range(self.r.shape[0]):
            gp = self.coordinate_converter.convert_from_coordinate(
                self.r[i, 1], self.r[i, 0]
            )
            sample, line = self.aoi.coordinate(gp)
            if (
                line >= 0
                and line < self.h.shape[0]
                and sample >= 0
                and sample < self.h.shape[1]
            ):
                self.h[int(line), int(sample)] = self.r[i, 2]
            if include_image:
                self.img1_h[int(round(self.r[i, 3])), int(round(self.r[i, 4]))] = (
                    self.r[i, 2]
                )
                self.img2_h[int(round(self.r[i, 5])), int(round(self.r[i, 6]))] = (
                    self.r[i, 2]
                )

        # For now, we'll also get the filled data. This will probably
        # go away to be replaced with something better in the future
        g = np.zeros((self.aoi.number_y_pixel, self.aoi.number_x_pixel, 2))
        ...
        if len(self.r) > 2:
            try:
                ...
                # a line
                self.h_fill = np.zeros(
                    (self.aoi.number_y_pixel, self.aoi.number_x_pixel)
                )
                self.h_fill[:] = fill_value

        else:
            self.h_fill = np.zeros((self.aoi.number_y_pixel, self.aoi.number_x_pixel))
            self.h_fill[:] = fill_value
        if include_image:
    ...
    def plot_res(self, aoi_img=None):
        ...
        if aoi_img is not None:
            ...
            for i in range(self.r.shape[0]):
                ...
                if (
                    ic.line >= 0
                    and ic.line < aoi_img.number_line
                    and ic.sample >= 0
                    and ic.sample < aoi_img.number_sample
                ):
                    y.append(ic.line)
                    x.append(ic.sample)
            plt.subplot(111)
    ...
    def range_image1(self):
        ...
        g4 = self.aoi.ground_coordinate(
            self.aoi.number_x_pixel, self.aoi.number_y_pixel
        )
        line = []
        sample = []
