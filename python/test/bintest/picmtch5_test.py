from geocal import IbisFile, VicarImageCoordinate
import pytest
import numpy as np
import numpy.testing as npt
from fixtures.require_check import require_vicar, require_vicarb
from fixtures.vicar_func import vicarb_run, require_picmtch5
...
def picmtch5_env(vicarb_env):
    ...
    vicarb_run(
        'f2 inp=picmtch5_xxim0 out=picmtch5_xxim1 func="(line*327+samp*193+line*samp)%256"'
    )
    vicarb_run("""+
...
def test_edge_case(picmtch5_env):
    ...
    d = np.zeros((5, 11))
    d[:, 0:2] = [[240, 238], [30, 30], [270, 270], [20, 20], [100, 100]]
...
""")
    ...
    _ = VicarImageCoordinate(f[1, 5], f[1, 6])
    _ = VicarImageCoordinate(f[2, 5], f[2, 6])
    _ = VicarImageCoordinate(f[3, 5], f[3, 6])
