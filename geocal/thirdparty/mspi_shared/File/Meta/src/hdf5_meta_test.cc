...
//-----------------------------------------------------------------------
// Test hdf5_export(H5::Group)
//-----------------------------------------------------------------------

    test_number++;
    {
      Hdf5Meta meta(meta_init());
      std::string filename = "File/Meta/tmp/hdf5_meta_test.hdf5";
...
//-----------------------------------------------------------------------
    ...
    test_number++;
    {
      ...
      Meta meta = Hdf5Meta::hdf5_import(file.openGroup("/").getLocId());

      
      std::string expect = 
	"3.14159265358979 : alpha_score\n"
	"File/Meta/tmp/hdf5_meta_test.hdf5 : filename\n"
	"1 : hungry\n"
	"Kanzi : name\n"
	"5 : number_banana\n"
	"-2 : score\n"
