...
class _GdalRasterImageHelper(object):
    ...
    def __get__(self, d):
        t = self.tre_class()
        t.read_from_tre_bytes(
            d["TRE", self.tre_name].encode("utf-8"), self.nitf_literal
        )
        return t
...
def _add_tre_to_gdal(tre, raster_name, help_desc, nitf_literal=False):
    ...
    setattr(
        GdalRasterImage, raster_name, property(h.__get__, h.__set__, None, help_desc)
    )
    setattr(
        GdalRasterImage, "has_" + raster_name, property(h.exists, None, None, help_desc)
    )
...
def _use00a_to_vicar(self, f):
    ...
    for field in self.field_names():
        f["GEOTIFF", "NITF_" + field.upper()] = self.get_raw_bytes(field).decode(
            "utf-8"
        )
...
def tre_use00a(fin, fout, creation_option):
    ...
    if "NITF_USE00A_ANGLE_TO_NORTH" in fin.raster_image(
        0
    ) or "NITF_ANGLE_TO_NORTH" in fin.raster_image(0):
        tre = TreUSE00A()
...
if have_pynitf:
    setattr(
        TreRPC00B,
        "is_rpc_a",
        property(tre_isnot_rpc_a, None, None, "Return true if this is type A"),
    )

    setattr(
        TreRPC00A,
        "is_rpc_a",
        property(tre_is_rpc_a, None, None, "Return true if this is type A"),
    )
