...
def _create_image_chip(
    ...
    with tempfile.TemporaryDirectory(dir=workdir) as tdir:
        ...
        gc4 = gdal_img_to_match.ground_coordinate(
            geocal_swig.ImageCoordinate(
                gdal_img_to_match.number_line - 1, gdal_img_to_match.number_sample - 1
            )
        )
        self.create_subset_file(
            f"{tdir}/ichip_landsat.tif",
            "gtiff",
            [gc1, gc2, gc3, gc4],
            None,
            "",
        ...
        gc1 = gdal_img_to_match.ground_coordinate(
            geocal_swig.ImageCoordinate(
                gdal_img_to_match.number_line // 2, gdal_img_to_match.number_sample // 2
            )
        )
        gc2 = gdal_img_to_match.ground_coordinate(
            geocal_swig.ImageCoordinate(
                gdal_img_to_match.number_line // 2 + 1,
        ...
        subprocess.run(
            f"gdalsrsinfo -o wkt {img_to_match_fname} > {tdir}/ichip.wkt",
            shell=True,
            check=True,
        )
        subprocess.run(
