...
def igc_mro_context(
    ...
    if lbl["INSTRUMENT_NAME"] != "CONTEXT CAMERA":
        raise RuntimeError(
            "igc_mro_context called for file %s which is actually INSTRUMENT_NAME %s"
    ...
    # handed
    klist = [kfile, kdat["ck_kernel"].kernel(tstart), kdat["spk_kernel"].kernel(tstart)]
    if kpfile:
        klist.append(kpfile)
    orb = SpicePlanetOrbit("MRO", "MRO_CTX", klist, PlanetConstant.MARS_NAIF_CODE)
    ...
    if subset is not None:
        ...
        nline = subset[2]
    img = SubRasterImage(
        img, sline, left_mask, nline, img.number_sample - (left_mask + right_mask)
    )
    ...
    tt = ConstantSpacingTimeTable(
        tstart, tstart + tspace * (img.number_line - 1), tspace
    )
    ...
    if img.number_sample != cam.number_sample(0):
        raise RuntimeError(
            f"The image has {img.number_sample} samples, but the context camera has {cam.number_sample(0)} samples. These need to match"
        )
...
def igc_mro_context_to_glas(igc_r):
    ...
    porb = PosCsephb(
        igc_r.ipi.orbit.orbit_underlying,
...
def igc_mex_hrsc(
    ...
    if lbl["INSTRUMENT_NAME"] != "HIGH RESOLUTION STEREO CAMERA":
        raise RuntimeError(
            "igc_mex_hrsc called for file %s which is actually INSTRUMENT_NAME %s"
            % (fname, lbl["INSTRUMENT_NAME"])
        )
    tstart = Time.parse_time(lbl["START_TIME"])
    ...
    if kpfile:
        klist.append(kpfile)
    orb = SpicePlanetOrbit("MEX", "MEX_HRSC_HEAD", klist, PlanetConstant.MARS_NAIF_CODE)
    img = GdalRasterImage(fname)
    ...
    igc = IpiImageGroundConnection(ipi, dem, img)
    if subset is not None:
        igc = OffsetImageGroundConnection(
            igc, subset[0], subset[1], subset[2], subset[3]
        )
    return igc
...
def igc_mro_hirise(
    ...
    if lbl["InstrumentId"] != "HIRISE":
        raise RuntimeError(
            "igc_mro_hirise called for file %s which is actually INSTRUMENT_NAME %s"
            % (fname, lbl["InstrumentId"])
        )
    ...
    if kpfile:
        klist.append(kpfile)
    orb = SpicePlanetOrbit(
        "MRO", "MRO_HIRISE_OPTICAL_AXIS", klist, PlanetConstant.MARS_NAIF_CODE
    )
    ...
    ccd_number = [0, 1, 2, 3, 12, 4, 10, 11, 5, 13, 6, 7, 8, 9][int(lbl["CpmmNumber"])]
...
__all__ = [
    "igc_mro_context",
    "igc_mro_context_to_glas",
    "igc_mex_hrsc",
    "igc_mro_hirise",
    "igc_mro_hirise_to_glas",
    "vicar_update_glas",
]
