...
from fixtures.require_check import require_hdf5, require_serialize
import numpy as np


def create_sample_file(unit_test_data):
    ...
    vel.attrs["Description"] = (
        "This is the velocity that goes with the position, in meter/second"
    )
    ...
    quat.attrs["Description"] = (
        "This is the attitude quaternion. This gives the orientation of the spacecraft with the ECI TOD position"
    )
...
def test_sc2rpc(unit_test_data):
    ...
    **NOTE** We get results that are about 7 meters different than what
    ...
    orb = HdfOrbit_EciTod_TimeAcs(str(unit_test_data / "sample_orbit.h5"))
    ...
    pt = orb.reference_surface_intersect_approximate(
        t, cam, FrameCoordinate(3375, 3648)
    )
...
def test_pickle(unit_test_data):
    ...
    pt2 = orb2.reference_surface_intersect_approximate(
        t, cam, FrameCoordinate(3375, 3648)
    )
