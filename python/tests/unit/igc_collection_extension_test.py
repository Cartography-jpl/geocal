...
import geocal_swig
from fixtures.require_check import geocal_test_data
...
def test_connection_igc(stereo_unit_test_data):
    ...
    igc3 = VicarImageGroundConnection(
        str(stereo_unit_test_data / "10MAY21-3.img"), demin
    )
    ...
    npt.assert_almost_equal(igc.image_coordinate(gp).line, ic.line, 2)
    npt.assert_almost_equal(igc.image_coordinate(gp).sample, ic.sample, 2)
    rpc = Rpc.generate_rpc(igc, -100, 100)
    npt.assert_almost_equal(rpc.image_coordinate(gp).line, ic.line, 2)
    npt.assert_almost_equal(rpc.image_coordinate(gp).sample, ic.sample, 2)


@pytest.mark.skip
def test_time(stereo_unit_test_data):
    demin = VicarLiteDem(str(stereo_unit_test_data / "nevada_elv_aoi.img"), True)
    igc1 = VicarImageGroundConnection(
        str(stereo_unit_test_data / "10MAY21-1.img"), demin
    )
    ...
    for i in range(100000):
        _ = igc_coll.image_coordinate(2, gp)


def test_igc_array(stereo_unit_test_data):
    demin = VicarLiteDem(str(stereo_unit_test_data / "nevada_elv_aoi.img"), True)
    igc1 = VicarImageGroundConnection(
        str(stereo_unit_test_data / "10MAY21-1.img"), demin
    )
    ...
    npt.assert_almost_equal(igc_coll.parameter_subset[0], igc1.parameter_subset[0], 10)
    npt.assert_almost_equal(igc_coll.parameter_subset[3], igc2.parameter_subset[1], 10)
    ic = ImageCoordinate(100, 200)
    gp = igc3.ground_coordinate(ic)
    ...
    assert (
        abs(
            igc_coll.image_coordinate_jac_cf(2, Ecr(gp))
            - igc3.image_coordinate_jac_cf(Ecr(gp))
        ).max()
        < 1e-6
    )
    igc_coll.parameter_subset = [1, 2, 3, 4, 5, 6]
    npt.assert_almost_equal(igc1.parameter_subset[0], 1, 10)
    npt.assert_almost_equal(igc2.parameter_subset[1], 4, 10)
    if False:
        ...
        jac = scipy.sparse.lil_matrix(
            (len(igc_coll.parameter_subset), len(igc_coll.parameter_subset))
        )
...
@pytest.mark.skip
def test_igc_array_with_igc_collection(stereo_unit_test_data):
    ...
    assert os.path.basename(igc_coll.image(0).file.file_name) == "10MAY21-1.img"
    assert os.path.basename(igc_coll.image(1).file.file_name) == "10MAY21-2.img"
    assert os.path.basename(igc_coll.image(2).file.file_name) == "10MAY21-3.img"
    assert (
        os.path.basename(igc_coll.image_ground_connection(0).image.file.file_name)
        == "10MAY21-1.img"
    )
    assert (
        os.path.basename(igc_coll.image_ground_connection(1).image.file.file_name)
        == "10MAY21-2.img"
    )
    assert (
        os.path.basename(igc_coll.image_ground_connection(2).image.file.file_name)
        == "10MAY21-3.img"
    )
    jac = scipy.sparse.lil_matrix(
        (len(igc_coll.parameter_subset), len(igc_coll.parameter_subset))
    ...
    npt.assert_almost_equal(jac[0, 4], 15387.7492279, 2)
    npt.assert_almost_equal(jac[1, 5], 17633.137014, 2)
...
def kepler_orbit_over_point(pt, cam, fc):
    ...
    def f(t):
        return (
            orb.reference_surface_intersect_approximate(orb.epoch + t, cam, fc).latitude
            - pt.latitude
        )
    ...
    def f2(ra):
        ...
        return (
            orb.reference_surface_intersect_approximate(
                orb.epoch + tpass, cam, fc
            ).longitude
            - pt.longitude
        )
...
def create_image_data(index):
    ...
    fout = VicarRasterImage(
        geocal_test_data + "igc_rolling_shutter_image%d.img" % index,
        "HALF",
        ras.number_line,
        ras.number_sample,
    )
...
def create_image_data_all():
    ...
    _ = pool.map(create_image_data, v)
...
def create_igc_collection_rolling_shutter():
    ...
    orb, tnadir = kepler_orbit_over_point(
        raster_aoi_pt,
        cam,
        FrameCoordinate(cam.number_line(0) / 2.0, cam.number_sample(0) / 2.0),
    )
    ...
    for i in range(20):
        ...
        tt = RollingShutterConstantTimeTable(t, t + cam.number_line(0) * tspace, tspace)
        title = "Image %d" % (i + 1)
    ...
    for i in range(20):
        ...
        ras = VicarLiteRasterImage(
            geocal_test_data + "igc_rolling_shutter_image%d.img" % i
        )
