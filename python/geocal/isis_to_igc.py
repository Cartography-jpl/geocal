...
from .spice_camera import ctx_camera, hirise_camera, lro_wac_camera, lro_nac_camera
...
class CtxIsisToIgc:
    def igc_to_glas(self, igc_r, focal_length):
        ...
        band = 0
        delta_sample = _adjust_delta_sample(100, igc_r.ipi.camera.number_sample(0))
        cam_g = GlasGfmCamera(
            igc_r.ipi.camera, band, delta_sample, "R", 0.65, focal_length * 1e-3
        )
        ipi_g = Ipi(
            orb_g,
            cam_g,
        ...
        **keywords,
        ...
        if match_isis:
            orb = SpicePlanetOrbit(
                "MRO", "MRO_CTX", klist, PlanetConstant.MARS_NAIF_CODE, "LT+S"
            )
        else:
            orb = SpicePlanetOrbit(
                "MRO", "MRO_CTX", klist, PlanetConstant.MARS_NAIF_CODE
            )
        ...
        if idata["LineExposureDuration"]["unit"] != "MSEC":
            raise RuntimeError(
                f"Not sure how to handle LineExposureDuration units of {idata['LineExposureDuration']['unit']}"
            )
        ...
        if not rsm:
            ...
            return (
                True,
                (igc, rsm_context(igc, min_height=min_height, max_height=max_height)),
            )
...
class HrscIsisToIgc:
    ...
    def isis_to_igc(
        ...
        **keywords,
        ...
        if match_isis:
            orb = SpicePlanetOrbit(
                "MEX", "MEX_HRSC_HEAD", klist_in, PlanetConstant.MARS_NAIF_CODE, "LT+S"
            )
        else:
            orb = SpicePlanetOrbit(
                "MEX", "MEX_HRSC_HEAD", klist_in, PlanetConstant.MARS_NAIF_CODE
            )
        tstart = Time.time_sclk(idata["SpacecraftClockCount"], "MRO")
        # 1e-3 is because LINE_EXPOSURE_DURATION is in milliseconds.
        if idata["LineExposureDuration"]["unit"] != "MSEC":
...
class LroCombineEvenOdd(CalcRaster):
    ...
    def calc(self, lstart, sstart):
        ...
        while ln < lstart + self.data.shape[0]:
            ...
            self.data[(ln - lstart) : (lend - lstart), :] = f_to_read.read_double(
                ln, sstart, lend - ln, self.data.shape[1]
            )
            self.data[(ln - lstart) : (lend - lstart), :][
                self.data[(ln - lstart) : (lend - lstart), :] < 0
            ] = 0
            ln = lend
...
class LroWacIsisToIgc:
    def igc_to_glas(self, igc_r, focal_length):
        ...
        band = 0
        delta_line = 2
        delta_sample = _adjust_delta_sample(16, igc_r.ipi.camera.number_sample(0))
        cam_g = GlasGfmCamera(
            igc_r.ipi.camera,
            band,
            delta_line,
    ...
    def isis_to_igc(
        ...
        **keywords,
        ...
        bname = re.sub(
            r"(\.vis)?(\.uv)?(\.even)?(\.cal)?\.cub$", "", isis_img.file_names[0]
        )
        ...
        if subset is not None:
            ...
            img = SubRasterImage(isis_img, sline, 0, nline, isis_img.number_sample)
        else:
            img = isis_img
        ...
        if match_isis:
            orb = SpicePlanetOrbit(
                "LRO", frame, klist, PlanetConstant.MOON_NAIF_CODE, "LT+S"
            )
        else:
            orb = SpicePlanetOrbit("LRO", frame, klist, PlanetConstant.MOON_NAIF_CODE)
        ...
        if idata["InterframeDelay"]["unit"] != "ms":
            raise RuntimeError(
                f"Not sure how to handle InterframeDelay units of {idata['InterframeDelay']['unit']}"
            )
        if idata["ExposureDuration"]["unit"] != "ms":
            raise RuntimeError(
                f"Not sure how to handle ExposureDuration units of {idata['InterframeDelay']['unit']}"
            )
        ...
        tt = ConstantSpacingFrameletTimeTable(tstart, tend, framelet_size, tspace)
...
class HiriseIsisToIgc:
    ...
    def igc_to_glas(self, igc_r, focal_length):
        ...
        delta_sample = _adjust_delta_sample(
            delta_sample, igc_r.ipi.camera.number_sample(0)
        )
        cam_g = GlasGfmCamera(
            igc_r.ipi.camera, band, delta_sample, "R", 0.7, focal_length * 1e-3
        )
        ipi_g = Ipi(
            orb_g,
            cam_g,
    ...
    def isis_to_igc(
        ...
        **keywords,
        ...
        ccd_number = [0, 1, 2, 3, 12, 4, 10, 11, 5, 13, 6, 7, 8, 9][
            int(idata["CpmmNumber"])
        ]
        tt = ConstantSpacingTimeTable(
            tstart, tstart + tspace * (img.number_line - 1), tspace
        )
        ...
        if not rsm:
            ...
            return (
                True,
                (igc, rsm_hirise(igc, min_height=min_height, max_height=max_height)),
            )
