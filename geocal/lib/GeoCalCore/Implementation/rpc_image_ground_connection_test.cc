...
BOOST_AUTO_TEST_CASE(basic)
{
  ...
  blitz::Array<double, 1> p0 = ig.parameter();
  BOOST_CHECK_CLOSE(ig.parameter()(0), ig.rpc().line_numerator[0], 1e-4);
  BOOST_CHECK_CLOSE(ig.parameter()(1), ig.rpc().line_numerator[2], 1e-4);
  BOOST_CHECK_CLOSE(ig.parameter()(2), ig.rpc().sample_numerator[0], 1e-4);
  BOOST_CHECK_CLOSE(ig.parameter()(3), ig.rpc().sample_numerator[5], 1e-4);
  BOOST_CHECK_CLOSE(ig.parameter()(4), ig.rpc().height_offset, 1e-4);
  BOOST_CHECK_EQUAL(ig.parameter_name()[0], "RPC Line Numerator Parameter 0");
  BOOST_CHECK_EQUAL(ig.parameter_name()[1], "RPC Line Numerator Parameter 2");
  BOOST_CHECK_EQUAL(ig.parameter_name()[2], "RPC Sample Numerator Parameter 0");
  BOOST_CHECK_EQUAL(ig.parameter_name()[3], "RPC Sample Numerator Parameter 5");
  BOOST_CHECK_EQUAL(ig.parameter_name()[4], "RPC Height Offset");
