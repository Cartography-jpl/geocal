...
def igccol():
    ...
    igc1 = geocal.OrbitDataImageGroundConnection(orb_data, cam, dem, img1, "Image 052")
    img2 = geocal.VicarLiteRasterImage("056.VIC")
    igc2 = geocal.OrbitDataImageGroundConnection(orb_data, cam, dem, img2, "Image 056")
...
def ppt_save(ppt_file):
    ...
    _ = slide.shapes.add_picture(img, Inches(0), Inches(0), height=Inches(7.5))
...
def test_sift_default(pdf_file, ppt_file, igccol):
    ...
    tp_gen = geocal.TiePointCollectFM(igccol, fdetect=fdetect, skip_ray_intersect=True)
...
def test_akaze_default(pdf_file, ppt_file, igccol):
    ...
    tp_gen = geocal.TiePointCollectFM(
        igccol, fdetect=fdetect, norm_type=cv2.NORM_HAMMING, skip_ray_intersect=True
    )
    tplist = tp_gen.tie_point_list()
