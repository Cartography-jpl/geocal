...
class Rsm(geocal_swig.with_parameter.WithParameterNested):
    ...
    def fit_data(self, Data, Nline, Nsamp):
        """

        void Rsm::fit_data(const blitz::Array< double, 2 > &Data, int Nline, int Nsamp) const
        ...
        """
        return _rsm.Rsm_fit_data(self, Data, Nline, Nsamp)


    def fill_in_ground_domain_vertex(self, *args):
        """
        ...
        """
        return _rsm.Rsm_fill_in_ground_domain_vertex(self, *args)


    def compare_data(self, Data):
        """

        blitz::Array< double, 2 > Rsm::compare_data(const blitz::Array< double, 2 > &Data) const
        After fitting, you can compare data as generated by generate_data
        against the rsm calculation.

        This return the difference in line and sample (true - calculated). A
        row is a nan if the x,y,z is out of range. 
        """
        return _rsm.Rsm_compare_data(self, Data)


    def compare_data_dist(self, Data):
        """

        blitz::Array< double, 1 > Rsm::compare_data_dist(const blitz::Array< double, 2 > &Data) const
        Like compare_data, but goes the other way and give a distance in
        meters. 
        """
        return _rsm.Rsm_compare_data_dist(self, Data)
    ...
    def generate_data(self, Igc, Min_height, Max_height, Nline_fit=20, Nsample_fit=20, Nheight_fit=5, Nsecond_pass_fit=20, Min_line=0, Max_line=-1, Min_sample=0, Max_sample=-1, Ignore_igc_error=True):
        """

        blitz::Array< double, 2 > Rsm::generate_data(const ImageGroundConnection &Igc, double Min_height, double
        Max_height, int Nline_fit=20, int Nsample_fit=20, int Nheight_fit=5,
        int Nsecond_pass_fit=20, int Min_line=0, int Max_line=-1, int
        Min_sample=0, int Max_sample=-1, bool Ignore_igc_error=true) const
        Generate data that can be used for fitting.

        This is useful if we are exploring how many sections to generate, or
        other logic. Because we use this at the python level, we return a
        blitz::Array. Each row is a data point, that is line, sample, x, y, z.

        By default we fit all the line/samples in the Igc, but you can select
        a subset if desired. Errors with the IGC can either be skipped, or
        case an exception depending on Ignore_igc_error

        We use the same algorithm used in RsmRationalPolynomial::fit. 
        """
        return _rsm.Rsm_generate_data(self, Igc, Min_height, Max_height, Nline_fit, Nsample_fit, Nheight_fit, Nsecond_pass_fit, Min_line, Max_line, Min_sample, Max_sample, Ignore_igc_error)
...
Rsm.fit_data = new_instancemethod(_rsm.Rsm_fit_data, None, Rsm)
Rsm.fill_in_ground_domain_vertex = new_instancemethod(_rsm.Rsm_fill_in_ground_domain_vertex, None, Rsm)
Rsm.compare_data = new_instancemethod(_rsm.Rsm_compare_data, None, Rsm)
Rsm.compare_data_dist = new_instancemethod(_rsm.Rsm_compare_data_dist, None, Rsm)
...
Rsm.generate_data = new_instancemethod(_rsm.Rsm_generate_data, None, Rsm)
