...
BOOST_FIXTURE_TEST_SUITE(rsm_rp_plus_grid, RsmFixture)
BOOST_AUTO_TEST_CASE(basic)
{
  RsmRpPlusGrid r(boost::make_shared<RsmRationalPolynomial>(3,3,3,3,3,3,3,3),
  ...
  for(int i = 0; i < jac.rows(); ++i)
    for(int j = 0; j < jac.cols(); ++j)
      BOOST_CHECK_CLOSE(jac(i,j), jac_exp(i,j), 1e-1);
}
...
BOOST_AUTO_TEST_CASE(serialize)
{
  if(!have_serialize_supported())
    return;
  boost::shared_ptr<RsmRpPlusGrid> r =
  ...
  if(false)
    std::cerr << d;
  boost::shared_ptr<RsmRpPlusGrid> rr = 
    serialize_read_string<RsmRpPlusGrid>(d);
  ImageCoordinate ic_expect = rpc.image_coordinate(gp);
