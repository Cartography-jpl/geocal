...
def test_job_database():
    ...
    for jid in jdb.keys():
        del jdb[jid]
    jid = jdb.add_job("job_database_dir", "job_database.log",
...
def test_handle_running_job():
    ...
    for jid in jdb.keys():
        del jdb[jid]
    jid_running = jdb.add_job("job_database_dir", "job_database.log",
                              ["ls"], 
                              start_if_available = False)
    jdb.set_status(jid_running, "running")
    jid_could_run = jdb.add_job("job_database_dir", "job_database.log",
                                ["ls"],
                                start_if_available = False)
