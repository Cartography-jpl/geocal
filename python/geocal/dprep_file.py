...
class DprepOrbit(OrbitArray_Eci_TimePgs):
    ...
    def __init__(
        self,
        eph_file_list: list[str | os.PathLike[str]],
        att_file_list: list[str | os.PathLike[str]],
    ) -> None:
        ...
        att_euler_angle = np.concatenate(
            [f.euler_angle for f in self.dprep_attitude_file], axis=0
        )
        ...
        for i, tm_v in enumerate(att_time):
            ...
            if tm >= orb_t.min_time and tm <= orb_t.max_time:
                ...
                att_quat2.append(
                    quaternion_to_array(
                        matrix_to_quaternion(
                            orb_to_eci_m @ quaternion_to_matrix(sc_to_orb_q)
                        )
                    )
                )
        super().__init__(
            eph_time, eph_pos, eph_vel, np.array(att_time2), np.array(att_quat2)
        )
