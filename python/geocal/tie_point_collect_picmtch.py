...
"""
    ...
    def post_run(self):
        ...
        self.res = [
            (
                ImageCoordinate(VicarImageCoordinate(ln1[i], smp1[i])),
...
class TiePointCollectPicmtch(object):
    def __init__(
        ...
        if (self.image_index2 >= 0 and self.ref_image_fname is not None) or (
            ...
            raise RuntimeError(
                "Need to either supply a ref_image_fname or an image_index2, but not both"
            )
        ...
        if surface_image_fname[0] is None:
            if map_info is not None:
                ...
                if surface_image_fname[1] is None:
                    raise RuntimeError(
                        "Need to either supply one of the surface_image_fname or a map_info to generate surface image"
                    )
                mi = VicarLiteRasterImage(
                    self.surface_image_fname[image_index2]
                ).map_info
    ...
    def _proj_img(self, index, mi, grid_spacing):
        ...
        self.surface_image_fname[index] = self.run_dir_name + "/img%d_surf.img" % index
        out = VicarRasterImage(
            self.surface_image_fname[index], igc_proj.map_info, "HALF"
        )
        out.close()
        m = mmap_file(self.surface_image_fname[index], mode="r+")
        m[:, :] = igc_proj.read(0, 0, igc_proj.number_line, igc_proj.number_sample)
