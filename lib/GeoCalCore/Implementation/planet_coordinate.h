...
//-----------------------------------------------------------------------
  ...
  virtual boost::shared_ptr<CartesianFixed> convert_to_cf(const Time& T) 
    const
  {
    boost::shared_ptr<PlanetFixed> res(new PlanetFixed(NAIF_CODE));
...
inline double PlanetFixed::height_reference_surface() const
{
  // Temp
