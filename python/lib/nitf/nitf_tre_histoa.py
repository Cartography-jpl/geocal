...
desc = ["HISTOA",
        ...
        [["loop", "f.nevents"],
         ...
         ["disp_flag", "Display-Ready Flag", 1, str],
         ["rot_flag", "Image Rotation", 1, int],
         ["rot_angle", "Angle Rotation", 8, str, {'condition' : "f.rot_flag == 1"}],
         ["asym_flag", "Image Projection", 1, str],
         ["zoomrow", "Mag in Line (row) Direction", 7, str, {'condition' : "f.asym_flag == '1'"}],
         ["zoomcol", "Mag in Element (column) Direction", 7, str, {'condition' : "f.asym_flag == '1'"}],
         ["proj_flag", "Asymmetric Correction", 1, str],
         ["sharp_flag", "Sharpening", 1, int],
         ["sharpfam", "Sharpening Family Number", 2, int, {'condition' : "f.sharp_flag == 1"}],
         ["sharpmem", "Sharpening Member Number", 2, int, {'condition' : "f.sharp_flag == 1"}],
         ["mag_flag", "Symmetrical Magnification", 1, int],
         ["mag_level", "Level of Relative Magnification", 7, str, {'condition' : "f.mag_flag == 1"}],
         ["dra_flag", "Dynamic Range Adjustment (DRA)", 1, int],
         ["dra_mult", "DRA Multiplier", 7, str, {'condition' : "f.dra_flag == 1"}],
         ["dra_sub", "DRA Subtractor", 5, int, {'condition' : "f.dra_flag == 1"}],
         ["ttc_flag", "Tonal Transfer Curve (TTC)", 1, int],
         ["ttcfam", "TTC Family Number", 2, int, {'condition' : "f.ttc_flag == 1"}],
         ["ttcmem", "TTC Member Number", 2, int, {'condition' : "f.ttc_flag == 1"}],
