...
if have_pynitf and not suppress_rsm:
    ...
    class RsmImageSegmentHook(NitfSegmentHook):
        ...
        def _rsm_add_rec(self, seg, v):
            if isinstance(v, RsmRpPlusGrid):
                ...
                if isinstance(v.section(0, 0), RsmRationalPolynomial):
                    ...
                    raise RuntimeError(
                        "Unknown section in RsmMultiSection " + str(v.section(0, 0))
                    )
        ...
        def before_write_hook(self, seg, nitf_file):
            ...
            seg.tre_list = [
                t for t in seg.tre_list if t.tre_tag not in self.rsm_tre_tag_list
            ]
            ...
            if seg.rsm:
                ...
                if seg.rsm.rsm_adjustable_parameter:
                    if isinstance(
                        seg.rsm.rsm_adjustable_parameter, RsmAdjustableParameterA
                    ):
                        t = TreRSMAPA()
                    else:
                        t = TreRSMAPB()
        ...
        def _rsm_find_tre_list(self, seg, tre_tag, edition, sec_row=None, sec_col=None):
            if sec_row is None:
                return [
                    i
                    for i in seg.tre_list
                    if i.tre_tag == tre_tag
                    and (edition is None or i.edition == edition)
                ]
            return [
                i
                for i in seg.tre_list
                if i.tre_tag == tre_tag
                and (edition is None or i.edition == edition)
                and i.row_section_number == sec_row
                and i.col_section_number == sec_col
            ]

        def _rsm_find_tre(self, seg, tre_tag, edition=None, sec_row=None, sec_col=None):
            t = self._rsm_find_tre_list(seg, tre_tag, edition, sec_row, sec_col)
            if len(t) > 1:
                raise RuntimeError(
                    "Found multiple TRE %s where only one expected" % tre_tag
                )
        ...
        def after_read_hook(self, seg, nitf_file):
            ...
            if self._rsm_find_tre(seg, "RSMPIA", edition) is not None:
                rsm_rp = self._rsm_find_tre(seg, "RSMPIA", edition).rsm_multi_section
                for i in range(rsm_rp.number_row_section):
                    for j in range(rsm_rp.number_col_section):
                        t = self._rsm_find_tre(
                            seg, "RSMPCA", edition, sec_row=i + 1, sec_col=j + 1
                        )
                        if t is None:
                            raise RuntimeError(
                                "Could find RSMPCA for (%d, %d)" % (i + 1, j + 1)
                            )
            ...
            if self._rsm_find_tre(seg, "RSMGIA", edition) is not None:
                ...
                for i in range(rsm_g.number_row_section):
                    for j in range(rsm_g.number_col_section):
                        t = self._rsm_find_tre(
                            seg, "RSMGGA", edition, sec_row=i + 1, sec_col=j + 1
                        )
                        if t is None:
                            raise RuntimeError(
                                "Could find RSMGGA for (%d, %d)" % (i + 1, j + 1)
                            )
                        rsm_g.section(i, j, t.rsm_grid)
            if (
                rsm_rp is None
                and self._rsm_find_tre(seg, "RSMPCA", edition) is not None
            ):
                rsm_rp = self._rsm_find_tre(
                    seg, "RSMPCA", edition
                ).rsm_rational_polynomial
            if rsm_g is None and self._rsm_find_tre(seg, "RSMGGA", edition) is not None:
