from geocal import Time, distance, Ecr, Geodetic
...
def test_time():
    ...
    npt.assert_almost_equal(t1.j2000, 64.18392726778984)
    t1 = Time.time_j2000(0.0)
    t2 = Time.time_j2000(100.0)
    npt.assert_almost_equal(t2 - t1, 100.0)
    npt.assert_almost_equal(t2 - (t1 + 100.0), 0)
    npt.assert_almost_equal(t2 - (100 + t1), 0)
    npt.assert_almost_equal((t2 - 100) - t1, 0)
    assert t1 == t1
    assert t1 < t2
    assert t1 <= t2
