...
if have_pynitf:
    ...
    if "NitfImageGdal" not in [
        h[0].__name__ for h in NitfSegmentDataHandleSet.default_handle_set()
    ]:
        NitfSegmentDataHandleSet.add_default_handle(NitfImageGdal, priority_order=-1)

    logger = logging.getLogger("nitf_diff")

    class ImageGdalDiff(NitfDiffHandle):
        def handle_diff(self, d1, d2, nitf_diff):
            if not isinstance(d1, NitfImageGdal) or not isinstance(d2, NitfImageGdal):
                return (False, None)
            is_same = True
            t1 = d1[:, :, :]
            t2 = d2[:, :, :]
            ...
            if not np.allclose(t1, t2):
                logger.difference("Image '%s is different", d1.subheader.iid1)
                is_same = False
            return (True, is_same)

    # Guard against adding multiple times
    if "ImageGdalDiff" not in [
        h[0].__class__.__name__ for h in NitfDiffHandleSet.default_handle_set()
    ]:
    ...
    def _gdal_multi_band(self):
        ...
        segindex = [
            t for t, iseg in enumerate(self.nitf_file.image_segment) if iseg is self
        ][0]
