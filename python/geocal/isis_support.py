...
def find_isis_kernel_file(f):
    ...
    if os.path.exists(f):
        return f
    spice_cache_dir = os.environ.get("SPICECACHE", os.path.expanduser("~/.spice_cache"))
    sublist = [
        (
            r"\$base",
    ...
    for r, _, s, s2, s3 in sublist:
        ...
        subprocess.run(
            [
                "rclone",
                "copy",
                f"--config={os.environ['ISISDATA']}/rclone.conf",
                t2,
                os.path.dirname(t),
            ],
...
def find_isis_kernel(klist, Skip_load=True):
    ...
    return SpiceKernelList(
        [find_isis_kernel_file(k) for k in klist.kernel_list], Skip_load
    )
...
class CtxPdsToIsis:
    ...
    def pds_to_isis(self, fin, pds_fname, isis_fname, pds_fname2=None):
        ...
        subprocess.run(
            [
                f"{os.environ['ISISROOT']}/bin/mroctx2isis",
                f"from={pds_fname}",
                f"to={isis_fname}",
            ],
            check=True,
        ...
        subprocess.run(
            [
                f"{os.environ['ISISROOT']}/bin/spiceinit",
                f"from={isis_fname}",
                "web=true",
            ],
            check=True,
...
class HrscPdsToIsis:
    def pds_to_isis(self, fin, pds_fname, isis_fname, pds_fname2=None):
        ...
        subprocess.run(
            [
                f"{os.environ['ISISROOT']}/bin/hrsc2isis",
                f"from={pds_fname}",
                f"to={isis_fname}",
            ],
            check=True,
        ...
        if False:
            ...
            if not os.path.exists(f"{os.environ['ISISDATA']}/mex/kernels/ck"):
                raise RuntimeError(
                    "HRSC kernels currently can't be handled by spiceinit over the web. Make sure you have separately downloaded the HRSC mex kernels from ISIS. Looking for the directory $ISISDATA/mex/kernels/ck"
                )
            subprocess.run(
                [f"{os.environ['ISISROOT']}/bin/spiceinit", f"from={isis_fname}"],
                check=True,
                stdout=subprocess.DEVNULL,
            )
        return (True, isis_fname)
...
class LroPdsToIsis:
    ...
    def pds_wac_to_isis(self, fin, pds_fname, isis_fname):
        subprocess.run(
            [
                f"{os.environ['ISISROOT']}/bin/lrowac2isis",
                f"from={pds_fname}",
                f"to={isis_fname}",
            ],
            check=True,
        ...
        if False:
            subprocess.run(
                [
                    f"{os.environ['ISISROOT']}/bin/lrowaccal",
                    f"from={t}.uv.odd.cub",
                    f"to={t}.uv.odd.cal.cub",
                ],
                check=True,
                stdout=subprocess.DEVNULL,
            )
    ...
    def pds_edr_nac_to_isis(self, fin, pds_fname, isis_fname):
        subprocess.run(
            [
                f"{os.environ['ISISROOT']}/bin/lronac2isis",
                f"from={pds_fname}",
                f"to={isis_fname}",
            ],
            check=True,
    ...
    def pds_cdr_nac_to_isis(self, fin, pds_fname, isis_fname):
        raise RuntimeError(
            "We don't currently support CDR format for NAC. Not sure if ISIS supports this, we'll need to dig a bit to see."
        )
        subprocess.run(
            [
                f"{os.environ['ISISROOT']}/bin/pds2isis",
                f"from={pds_fname}",
                f"to={isis_fname}",
            ],
            check=True,
...
class DummyPdsToIsis:
    ...
    def pds_to_isis(self, fin, pds_fname, isis_fname, pds_fname2=None):
        ...
        if not os.path.exists(isis_fname):
            ...
            raise RuntimeError(
                f"File {isis_fname} already exists when trying to import {pds_fname}"
            )
        return (True, isis_fname)
...
class HirisePdsToIsis:
    ...
    def pds_to_isis(self, fin, pds_fname, isis_fname, pds_fname2=None):
        if (
            fin["INSTRUMENT_ID"] != "HIRISE" and fin["INSTRUMENT_ID"] != '"HIRISE"'
        ) or pds_fname2 is None:
            return (False, None)
        setup_isis()
        fbase, _ = os.path.splitext(isis_fname)
        subprocess.run(
            [
                f"{os.environ['ISISROOT']}/bin/hi2isis",
                f"from={pds_fname}",
                f"to={fbase}_f1.cub",
            ],
            check=True,
        ...
        subprocess.run(
            [
                f"{os.environ['ISISROOT']}/bin/hical",
                f"from={fbase}_f1.cub",
                f"to={fbase}_f1_cal.cub",
            ],
            check=True,
            stdout=subprocess.DEVNULL,
        )
        ...
        subprocess.run(
            [
                f"{os.environ['ISISROOT']}/bin/histitch",
...
__all__ = [
    "setup_isis",
    "read_kernel_from_isis",
    "find_isis_kernel_file",
    "find_isis_kernel",
    "pds_to_isis",
    "PdsToIsisHandleSet",
]
