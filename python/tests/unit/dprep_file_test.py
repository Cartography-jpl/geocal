from geocal import DprepEphemerisFile, DprepAttitudeFile, DprepOrbit, rad_to_deg, KeplerOrbit, Time, distance
...
def test_dprep_orbit(unit_test_data):
    ...
    korb = KeplerOrbit(Time.min_valid_time, Time.max_valid_time, kepoch,
                       a, e, inc * rad_to_deg, an * rad_to_deg, ap * rad_to_deg,
                       ma * rad_to_deg)
    for i in range(0, 1000, 100):
        print(distance(orb.position_cf(orb.min_time + i),korb.position_cf(orb.min_time + i)))
