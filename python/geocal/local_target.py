...
class InLocalTarget(Target):
    ...
    def prepare(self):
        ...
        if not os.path.exists(self.lfname):
            subprocess.run(
                ["rsync", "--copy-links", self.fname, self.lfname], check=True
            )
...
class OutLocalTarget(Target):
    ...
    def __init__(self, fname, local_directory, generating_extension=".generating"):
    ...
    def finish(self):
        ...
        subprocess.run(["rsync", "--links", self.lfname, self.fname], check=True)
    ...
    def finish_error(self):
        ...
        try:
            ...
            if self.local_directory is not None:
                subprocess.run(
                    ["rsync", "--links", self.lfname + ".error", self.fname + ".error"]
                )
        except OSError:
...
class OutTempLocalTarget(Target):
    ...
    *not* copy the file out once we are done. Instead, we allow the file
    ...
    def remove_temporary(self):
        ...
        for f in (self.lfname, self.lfname + ".generating", self.lfname + ".error"):
            try:
                os.unlink(f)
            except OSError:
...
class OutTempDirLocalTarget(Target):
    ...
    def __init__(self, dname, local_directory):
        ...
        self.gfname = (
            os.path.dirname(self.lfname)
            + "_generating/"
            + os.path.basename(self.lfname)
        )
        self.efname = (
            os.path.dirname(self.lfname) + "_error/" + os.path.basename(self.lfname)
        )
...
class GeneratedOutLocalTarget(Target):
    ...
    def __init__(self, fname, local_directory, generating_name=None):
        ...
        if self.local_directory is not None:
            self.generating_name = os.path.abspath(
                local_directory + self.generating_name
            )
