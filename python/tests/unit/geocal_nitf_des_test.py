...
def test_poscsephb(isolated_dir):
    ...
    npt.assert_almost_equal(p.time_step, p2.time_step)
    ...
    for t in np.linspace(0, 100.0, 200):
        assert (
            distance(
                orb1.position_cf(orb1.min_time + t), orb2.position_cf(orb2.min_time + t)
            )
            < 0.01
        )
...
def test_attcsattb(isolated_dir):
    ...
    npt.assert_almost_equal(att.time_step, att2.time_step)
