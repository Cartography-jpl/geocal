...
def rsm_plot_diff(r, igc, fname=None, min_height=-5000, max_height=-1500):
    ...
    ln, smp, lncalc, smpcalc, distance_true_vs_calc = r.compare_igc(
        igc, igc.number_line, 5, dem_nominal
    )
    print(
        "Max line diff:   ",
...
def rsm_context(igc, diagnostic=False, min_height=-5000, max_height=-1500):
    ...
    if diagnostic:
        ...
        pts = SpiceHelper.boresight_and_footprint(
            igc.pixel_time(ic), PlanetConstant.MARS_NAIF_CODE, "MRO", "MRO_CTX"
        )
        print(
            "Distance spice boresight and IGC: ",
    ...
    r = Rsm(
        RsmRpPlusGrid(
            RsmRationalPolynomial(5, 5, 3, 1, 1, 1, 5, 1, 40, 40, 20, 40),
            RsmGrid(3, 1000, 50),
        ),
...
def rsm_hrsc(igc, diagnostic=False, min_height=-5000, max_height=-1500):
    ...
    if diagnostic:
        ...
        ic = ImageCoordinate(200, igc.number_sample / 2)
        pts = SpiceHelper.boresight_and_footprint(
            igc.pixel_time(ic), PlanetConstant.MARS_NAIF_CODE, "MEX", "MEX_HRSC_NADIR"
        )
        print(
            "Distance spice boresight and IGC: ",
            distance(pts[0], igc.ground_coordinate(ic)),
    ...
    r = Rsm(
        RsmRpPlusGrid(
            RsmMultiSection(
                ...
                3,
                1,
                RsmRationalPolynomial(5, 5, 3, 1, 1, 1, 5, 1, 40, 40, 20, 40, False),
            ),
...
def rsm_hirise(igc, diagnostic=False, min_height=-5000, max_height=-1500):
