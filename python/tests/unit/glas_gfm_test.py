...
def rsm_plot_diff(r, igc, fname=None, min_height=-5000, max_height=-1500):
    ...
    ln, smp, lncalc, smpcalc, distance_true_vs_calc = r.compare_igc(
        igc, 1000, 5, dem_nominal
    )
    print(ln)
    print(lncalc)
    print(
        "Max line diff:   ",
        np.nanmax(abs(ln - lncalc)),
        " at ",
        np.unravel_index(np.nanargmax(abs(ln - lncalc)), ln.shape),
    )
    print(
        "Max sample diff: ",
        np.nanmax(abs(smp - smpcalc)),
        " at ",
...
def test_rip_glas(nitf_sample_rip):
    ...
    print(glas)
    ref_pt = Ecr(
        glas.tre_csexrb.ground_ref_point_x,
    ...
    for i in range(0, igc2.number_line, 20):
        for j in range(0, igc2.number_sample, 20):
            ...
            d1 = distance(igc1.ground_coordinate(ic), igc2.ground_coordinate(ic))
            d2 = distance(igc3.ground_coordinate(ic), igc2.ground_coordinate(ic))
...
def test_create_glas(nitf_sample_rip, isolated_dir):
    ...
    f2 = pynitf.NitfFile("glas_test.ntf")
    ...
    for i in range(0, igc1.number_line, 20):
        for j in range(0, igc1.number_sample, 20):
            ic = ImageCoordinate(i, j)
            d1 = distance(igc1.ground_coordinate(ic), igc2.ground_coordinate(ic))
            d2 = distance(igc1.ground_coordinate(ic), igc3.ground_coordinate(ic))
            max_diff1 = max(d1, max_diff1)
            max_diff2 = max(d2, max_diff2)
    print(max_diff1)
    print(max_diff2)
...
def test_create_multiple_fa_glas(nitf_sample_rip, isolated_dir):
    ...
    # in our calculation
    igc1.ipi.camera.field_alignment = np.array(
        [[-0.00765, 0, 0, 0], [0, 0.00765 / 128 * 2, 0.00765, 0.00765 / 128 * 2]]
    )
    f.image_segment[0].create_glas_gfm(igc1)
    # Turn off refraction in MSP calculation. Normally we want this on,
    # but we turn it off so we can get better agreement with our Igc.
...
def test_create_gfm(isolated_dir, igc_gfm):
    ...
    f2 = pynitf.NitfFile("gfm_test.ntf")
    with open("f2.txt", "w") as fh:
        print(f2, file=fh)
    ...
    for i in range(0, igc_gfm.number_line, 20):
        for j in range(0, igc_gfm.number_sample, 20):
            ic = ImageCoordinate(i, j)
            d1 = distance(igc_gfm.ground_coordinate(ic), igc2.ground_coordinate(ic))
            d2 = distance(igc_gfm.ground_coordinate(ic), igc3.ground_coordinate(ic))
...
@pytest.mark.skip
def test_create_steep_igc(isolated_dir, igc_gfm, unit_test_data):
    ...
    q = quat_rot_x(0.0) * quat_rot_y(58.0 * deg_to_rad) * quat_rot_x(-2.7 * deg_to_rad)
...
@require_msp
@require_pynitf
def test_refraction_gfm(isolated_dir, igc_gfm, unit_test_data):
    ...
    if False:
        ...
        print(poff)
        od2 = QuaternionOrbitData(
            od, [poff[0], poff[1], poff[2]], Quaternion_double(1, 0, 0, 0)
        )
    ...
    print(
        "Single point 1: ",
        distance(igc2.ground_coordinate(ic), igc_gfm.ground_coordinate(ic)),
    )
    print(
        "Single point 2: ",
        distance(igc2.ground_coordinate(ic), igc3.ground_coordinate(ic)),
    )
    for i in range(0, igc_gfm.number_line, 20):
        for j in range(0, igc_gfm.number_sample, 20):
            ic = ImageCoordinate(i, j)
            d1 = distance(igc_gfm.ground_coordinate(ic), igc2.ground_coordinate(ic))
            d2 = distance(igc_gfm.ground_coordinate(ic), igc3.ground_coordinate(ic))
            d3 = distance(igc2.ground_coordinate(ic), igc3.ground_coordinate(ic))
            max_diff1 = max(d1, max_diff1)
            max_diff2 = max(d2, max_diff2)
...
def test_create_many_gfm(isolated_dir, igc_gfm):
    ...
    f2 = pynitf.NitfFile("gfm_large_number.ntf")
    with open("f2.txt", "w") as fh:
        print(f2, file=fh)
    for iseg in f2.image_segment:
        ...
        assert (
            distance(igc_gfm.ground_coordinate(ic), igc2.ground_coordinate(ic)) < 1e-2
        )
...
def test_gfm_nitfdiff(isolated_dir, igc_gfm):
    ...
    subprocess.run(
        "NITF_PLUGIN=geocal nitf_diff gfm_test1.ntf gfm_test2.ntf", shell=True
    )
...
def test_create_from_scratch_glas(isolated_dir, igc_half_meter_pushbroom):
    ...
    cam_g.field_alignment_fit(igc_r.ipi.camera, igc_r.ipi.camera.number_sample(0))
    ipi_g = Ipi(
        orb_g,
        cam_g,
        0,
        igc_r.ipi.time_table.min_time,
        igc_r.ipi.time_table.max_time,
        igc_r.ipi.time_table,
    )
    ...
    for i in range(0, 2048, 100):
        for j in range(0, 2048, 100):
            assert (
                distance(
                    igc_r.ground_coordinate(ImageCoordinate(i, j)),
                    igc_g.ground_coordinate(ImageCoordinate(i, j)),
                )
                < 0.01
            )
    f = pynitf.NitfFile()
    ...
    for i in range(0, 2048, 100):
        for j in range(0, 2048, 100):
            assert (
                distance(
                    igc_r.ground_coordinate(ImageCoordinate(i, j)),
                    igc_g2.ground_coordinate(ImageCoordinate(i, j)),
                )
                < 0.05
            )
...
def test_create_staring_glas(isolated_dir, igc_staring):
    ...
    tdelta = (
        igc_r.ipi.time_table.time(ImageCoordinate(1, 0))[0]
...
def test_create_reverse_staring_glas(isolated_dir, igc_staring):
    ...
    tt2 = ConstantSpacingTimeTable(
        tt.time(ImageCoordinate(igc_r.number_line - 1, 0))[0],
        tt.time(ImageCoordinate(0, 0))[0],
        -tt.time_space,
    )
...
@pytest.mark.skip
def test_create_dataset_refraction(igc_gfm, igc_staring, isolated_dir, unit_test_data):
