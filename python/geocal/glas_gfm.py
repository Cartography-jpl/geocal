...
class GlasGfm(object):
    ...
    def create(iseg, igc, refraction_flag=None, velocity_aberration_flag=None):
        ...
        if isinstance(igc, IpiImageGroundConnection):
            ...
            t.day_first_line_image, t.time_first_line_image = (
                geocal_time_to_nitf_date_second_field(min_time)
            )
        ...
        if refraction_flag is not None:
            ...
            if hasattr(igc, "refraction") and igc.refraction is not None:
                t.atm_refr_flag = 1
            else:
                t.atm_refr_flag = 0
        if velocity_aberration_flag is not None:
            ...
            if vabb is not None and isinstance(
        ...
        # empty one
        if hasattr(igc, "glas_gfm_covariance") and igc.glas_gfm_covariance is not None:
    ...
    def igc(
        ...
        dem=SimpleDem(),
        naif_code=None,
        velocity_aberration_exact=False,
    ):
        ...
        if self.tre_csexrb.sensor_type in ("S", "P"):
            ...
            igc = IpiImageGroundConnection(ipi, dem, None, self.iseg.iid1)
        elif self.tre_csexrb.sensor_type == "F":
            if self.tre_csexrb.time_stamp_loc != 0:
                raise RuntimeError(
                    "We don't handle having the time stamps in a MTIMSA TRE"
                )
            t = (
                timestamp_to_geocal_time(self.tre_csexrb.base_timestamp)
                + self.tre_csexrb.dt_multiplier * 1e-9 * self.tre_csexrb.dt[0]
            )
            igc = OrbitDataImageGroundConnection(
                orb,
                t,
                cam,
                dem,
    ...
    def time_table(self):
        ...
        if self.tre_csexrb.day_first_line_image is None:
            return None
        tstart = nitf_date_second_field_to_geocal_time(
            self.tre_csexrb.day_first_line_image, self.tre_csexrb.time_first_line_image
        )
        ...
        if self.tre_csexrb.sensor_type == "S":
            ...
            return ConstantSpacingFrameletTimeTable(
                tstart, tend, nline_framelet, (tend - tstart) / (nframelet - 1)
            )
        else:
            raise RuntimeError("Unrecognized tre_csexrb sensor type")
    ...
    def find_one_des(self, desid):
        ...
        if len(res) > 1:
            raise RuntimeError(
                "Found more than one possible DES %s in GLAS/GFM" % desid
            )
        return res[0]
...
class _CameraForGlas(Camera):
    def __init__(
        ...
        self.slv = scipy.interpolate.RegularGridInterpolator(
            ...
            np.array(
                [
                    lv_arr[:, 0] * (focal_length / lv_arr[:, 2]),
...
def _create_glas_from_sc_look_vector(
    cls,
    sclv_list,
    focal_length=1,
    ...
    lv_arr = np.array(
        [
            [sclv.look_vector[0], sclv.look_vector[1], sclv.look_vector[2]]
            for sclv in sclv_list
        ]
    )
    cfit = _CameraForGlas(
        gcam.number_sample(0),
        lv_arr,
        gcam.focal_length,
...
def _create_gfm_from_look_vector(
    cls,
    lv_arr,
    focal_length=1,
    focal_length_time=Time.parse_time("2020-01-01T00:00:00Z"),
    ...
    gcam = cls(
        line_number_first + delta_line_pair * lv_arr.shape[0],
        sample_number_first + delta_sample_pair * lv_arr.shape[1],
    )
    gcam.focal_length = focal_length
    gcam.focal_length_time = focal_length_time
    gcam.first_line_block = [
        line_number_first,
    ]
    gcam.first_sample_block = [
        sample_number_first,
    ]
    gcam.delta_line_block = [
        delta_line_pair,
    ]
    gcam.delta_sample_block = [
        delta_sample_pair,
    ]
    gcam.band_type = band_type
    gcam.band_wavelength = band_wavelength
...
def _create_glas_from_field_angle(cls, fa_x, fa_y, **keyword):
    ...
    sclv_list = [
        ScLookVector.create_from_field_angle(fa_x[i], fa_y[i]) for i in range(len(fa_x))
    ]
    return cls.create_glas_from_sc_look_vector(sclv_list, **keyword)


GlasGfmCamera.create_glas_from_sc_look_vector = classmethod(
    _create_glas_from_sc_look_vector
)
