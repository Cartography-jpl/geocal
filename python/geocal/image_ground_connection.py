...
class GdalImageGroundConnection(geocal_swig.RpcImageGroundConnection):
    ...
    def __init__(
        ...
        geocal_swig.RpcImageGroundConnection.__init__(
            self, rpc, dem, img, title, image_mask, ground_mask, fit_height_offset
        )
...
class VicarImageGroundConnection(geocal_swig.RpcImageGroundConnection):
    ...
    def is_vicar_file(cls, fname):
        ...
        if m:
            ...
            baseend = m.group(4)
            i = int(m.group(2))
            return geocal_swig.VicarLiteFile.is_vicar_file(
                "%s%d%s" % (base, i, baseend)
            )
        else:
...
def _view_angle(self, image_coordinate=None, delta_h=100):
    ...
    if image_coordinate is None:
        image_coordinate = geocal_swig.ImageCoordinate(
            self.number_line / 2.0, self.number_sample / 2.0
        )
...
def _footprint_geometry(self, cconver=geocal_swig.GeodeticConverter()):
    ...
    for pt in [
        ...
        geocal_swig.ImageCoordinate(self.image.number_line, self.image.number_sample),
...
def _gsd_values(self, image_coordinate=None, height=None):
    ...
    if image_coordinate is None:
        image_coordinate = geocal_swig.ImageCoordinate(
            self.number_line // 2, self.number_sample // 2
        )
    if height is None:
        hlist = []
        for i in (-1, 0, 1):
    ...
    gc2 = self.ground_coordinate_approx_height(
        geocal_swig.ImageCoordinate(image_coordinate.line + 1, image_coordinate.sample),
        height,
    )
    gc3 = self.ground_coordinate_approx_height(
        geocal_swig.ImageCoordinate(image_coordinate.line, image_coordinate.sample + 1),
        height,
    )
