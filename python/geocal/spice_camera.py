...
def hrsc_camera():
    ...
    principal_point = FrameCoordinate(
        center_y - 0.5 + bore_y / pitch_y,
        center_x - 0.5 - left_shield + bore_x / pitch_x,
    )
    hrsc_cam = QuaternionCamera(
...
def hirise_camera(ccd_number, bin_mode=1, tdi_mode=128):
    ...
    return CameraRadialDistortionAndTransform(
        Quaternion_double(1, 0, 0, 0),
        od_k,
        1,
        nsamp,
...
def ctx_camera(start_sample=0, nsamp=None):
    ...
    ctx_cam = CameraRadialDistortion(
        Quaternion_double(1, 0, 0, 0),
        od_k,
        1,
        nsamp + abs(start_sample),
        pitch,
        pitch,
        focal_length,
        principal_point,
...
def lro_nac_camera(typ="left", spatial_summing=1):
    ...
    nac_cam = CameraRadialDistortionAndTransform(
        Quaternion_double(1, 0, 0, 0),
        od_k,
        1,
        nsamp / bin_mode,
...
def lro_wac_camera(band=3, mode="COLOR"):
    ...
    tinv_off = np.dot(tinv_m, -t_off)
    bin_mode = 1
    wac_cam = CameraRadialDistortionAndTransform(
        Quaternion_double(1, 0, 0, 0),
        od_k,
        sub_nline,
        nsamp - 2 * soff,
