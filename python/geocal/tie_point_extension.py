...
def _tp__write_old_mspi_format(self, dirname, gcp_sigma=5):
    ...
    with open(filename, "w") as f:
        ...
        f.write(
            "%0.17e %0.17e %0.17e %0.17e 0.00000000000000000e+00 0.00000000000000000e+00 0.00000000000000000e+00 %0.17e 0.00000000000000000e+00 0.00000000000000000e+00 0.00000000000000000e+00 %0.17e %d %d : ground_loc%d # %0.6f N %0.6f E\n"
            % (
                self.ground_location.position[0],
                self.ground_location.position[1],
                self.ground_location.position[2],
                tp_cov,
                tp_cov,
                tp_cov,
                1 if self.is_gcp else 0,
                self.number_image,
                self.id,
                self.ground_location.latitude,
                self.ground_location.longitude,
            )
        )
        for i in range(self.number_image):
            if self.image_coordinate(i) is None:
                f.write(
                    "0.00000000000000000e+00 0.00000000000000000e+00 0.00000000000000000e+00 0.00000000000000000e+00 0.00000000000000000e+00 0.00000000000000000e+00 0 : image_loc%d_view%d\n"
                    % (self.id, i)
                )
            else:
                ...
                f.write(
                    "%0.17e %0.17e %0.17e 0.00000000000000000e+00 0.00000000000000000e+00 %0.17e 1 : image_loc%d_view%d\n"
                    % (
                        ic.line,
                        ic.sample,
                        ln_sigma * ln_sigma,
...
def _tp_display(
    ...
    for i in range(self.number_image):
        if self.image_coordinate(i) is None:
            ...
            plt.title(igc_coll.title(i))
            if map_info:
                igc_proj = IgcMapProjected(
                    map_info, igc_coll.image_ground_connection(i)
                )
                pt = igc_proj.coordinate(
                    igc_coll.ground_coordinate(i, self.image_coordinate(i))
                )
                igc_proj.display(pt, sz)
            elif surface_image:
                pt = surface_image[i].coordinate(
                    igc_coll.ground_coordinate(i, self.image_coordinate(i))
                )
    ...
    for tpcol in args:
        for tp in tpcol:
            ...
            for i in range(tp.number_image):
                ...
                if ic:
                    mytp.image_coordinate(
                        i + ioff, ic, tp.line_sigma(i), tp.sample_sigma(i)
                    )
            res.append(mytp)
...
def _tpcol_gcp_diff(self, tpcol_other):
    ...
    for i, tp in enumerate(self):
        if tp.is_gcp:
            ...
            clv = CartesianFixedLookVector(
                tp.ground_location, tpcol_other[i].ground_location
            )
            ...
            move_distance.append(
                distance(tp.ground_location, tpcol_other[i].ground_location)
            )
    return pd.DataFrame(
        {
            "latitude": lat,
    ...
    for i, tp in enumerate(self):
        ...
        if iloc:
            try:
                ...
                data[i, 6] = distance(
                    d.surface_point(tp.ground_location),
...
def _tpcol_data_frame2(self, ref_image=None):
    ...
    header = ["ID", "Is_GCP", "Longitude (deg)", "Latitude (deg)", "Height (m)"]
    if ref_image is not None:
    ...
    df = pd.DataFrame(data=self.data_array(ref_image), columns=header)
