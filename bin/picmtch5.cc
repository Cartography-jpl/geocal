...
void refine(float corr[3][3],float* vloff,float* vsoff,int* ireferr)
{
   int i,j,iq,ierror;
...
float getzvl(float* a,int n,float coord[2],int nw,int nr)
{
...
void rfit(int ilin,int jsmp,float* vmax,float* vloff,float* vsoff,
	  float corr[3][3],int nohpf,int srchdim,int fftsize,int search)
{
...
void getgrid(int filenum,float* chip,int narray,int ndim,double trans[12],
	     float centrx,float centry,float rmag[2],int* chop,
	     float pcl,float pcs,int zerothr)
{
   float *gridx,*gridy;
   short int **imbuf;
   ...
   if (narray%2==1||ndim%2==1) zmabend((char*) "getgrid error");
   lnl = lnlg[filenum-1];
   lns = lnsg[filenum-1];
...
void throwout(int* throwcount,float** a,float** b,int* neq,int neqmax)
{
...
int main(int Argc, char *Argv[])
{
   ...
   if (itiecount!=0&&itiecount!=6) zmabend((char*)"itie/otie count must be 0 or 6");
   zvparmd((char*)"magnify",rmagtae,&pcount,&pdef,2,0);
   if (pcount==1) zmabend((char*)"magnify needs 2 inputs, or default = no inputs"); 
   zvparm((char*)"MAGSHRK",magshrk,&pcount,&pdef,1,2);
   zvparmd((char*)"magmin",rmagmin,&pcount,&pdef,2,0);
   zvparmd((char*)"thr_res",&thr_res,&pcount,&pdef,1,0);
   zvparmd((char*)"angoff",&angoff,&pcount,&pdef,1,0);
   angoff /= 57.29577;
   zvp((char*)"search",&search,&pcount);
   zvp((char*)"minsrch",&msrc,&pcount);
   zvp((char*)"fftsize",&fftsize,&pcount);
   zvp((char*)"zerothr",&zerothr,&pcount);
   if (search%2==1) search++;
   if (minsrch%2==1) minsrch++;
   if (fftsize%2==1) fftsize++;
   predfunc = zvptst((char*)"QUAD");
   zvp((char*)"redo",&nredo,&pcount);
   if (msrc<fftsize) msrc = fftsize;
   if (search<fftsize) search = fftsize;
   if (search<msrc) search = msrc;
   
   nohpftae = zvptst((char*)"nohpf");
   subpix = !zvptst((char*)"nosubpix");
   geocord1 = zvptst((char*)"geocord1");
   geocord2 = zvptst((char*)"geocord2");
   
   zvp((char*)"zwind",&getw,&pcount);
   zvp((char*)"zreject",&getr,&pcount);
   zvp((char*)"zerolim",&zerolim,&pcount);  zerolim *= 0.01;
   zvp((char*)"zerolim2",&zerolim2,&pcount);
   if (zerolim2<-0.5) zerolim2 = zerolim*100.0;
   zerolim2 *= 0.01;
   
   zvp((char*)"retry",&retryparm,&pcount);
   nretry = (int)retryparm[0]+1;
   rretry = (double)retryparm[1];
   tretry = (double)retryparm[2];
   zvp((char*)"ffthalf",&ffthalf,&pcount);
   if (ffthalf>0)
      {
      if (nretry>1) zmabend((char*)"can't use both RETRY and FFTHALF parameters");
      nretry = ffthalf+1;
      }
   
   zvp((char*)"fitmin",&rmcor,&pcount);
   zvp((char*)"autofit",&autofit,&pcount);
   if (autofit>0&&autofit<12)
      zmabend((char*)"autofit must be at least 12; or zero (autofit off)");
   if (autofit>0) 
      {
   ...
   nohpf = nohpftae;
   if (getw%2==0) zmabend((char*)"zwind must be odd");
   retry = (nretry>1);
   if (retry&&gcpf) zmabend((char*)"can't retry gcpf");
   ...
   if (ncol<11) zmabend((char*)"ibis file needs 11 columns");
   
   /* malloc arrays, read in the cols (if not gcpf) */

   if (gcpf) zmabend((char*)"ground control point file not implemented yet");
   ...
   if (!geocord1)
      ...
      if (status!=1) zmabend((char*)"Failed to get GeoTIFF label, first input");
      status = geofix(labelstr,t,tinv,labnl,labns,corner);
      if (status!=1)
         zmabend((char*)"Failed to get mapping from GeoTIFF label, first input");
      }
   if (itiecount==0)
      {
   ...
   if (otiecount==6&&!geocord2)
      ...
      if (status!=1) zmabend((char*)"Failed to get GeoTIFF label, second input");
      status = geofix(labelstr,u,uinv,labnl,labns,corner);
      if (status!=1)
         zmabend((char*)"Failed to get mapping from GeoTIFF label, second input");
      }
   if (otiecount==0)
      {
      if (!geocord1)
