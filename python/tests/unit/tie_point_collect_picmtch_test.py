...
@require_vicar_gdalplugin
@require_vicar
def test_tp_picmtch(isolated_dir, stereo_unit_test_data):
    ...
    subprocess.check_call(
        ["gdal_translate", "-q", "-of", "VICAR", str(surfimg_fname1), "img1.img"]
    )
    subprocess.check_call(
        ["gdal_translate", "-q", "-of", "VICAR", str(surfimg_fname2), "img2.img"]
    )
    tpcollect = TiePointCollectPicmtch(
        igc_coll,
        ["img1.img", "img2.img"],
...
def test_mars_tp_picmtch(isolated_dir):
    ...
    tpcollect = TiePointCollectPicmtch(
        igc_coll, [img1, img2], ref_image_fname=img2, log_file="tpcol.log", quiet=False
    )
    ...
    print(tpcol)
    warp_image(
        img1, "ctx1_full_corr_6m.img", img2_res, tpcol, nah=1000, nav=1000, quiet=False
    )
    subprocess.run(
        [
            bdir + "../mygdalcompare.py",
            "ctx1_full_corr_6m.img",
...
def test_mars_search(isolated_dir):
    ...
    reference_image = (
        "/raid26/tllogan/mars_map/hirise_gale/5degfix/ESP_025012_1745_RED.hlf"
    )
    rsm = img.rsm
