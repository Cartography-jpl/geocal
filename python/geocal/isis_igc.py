...
class IsisIgc(ImageGroundConnection):
    ...
    def ground_coordinate_dem(self, ic, d):
        ...
        subprocess.run(
            [
                f"{os.environ['ISISROOT']}/bin/campt",
                f"from={self.fname}",
                "to=campt.csv",
                "type=IMAGE",
                f"line={str(ic.line + 0.5)}",
    ...
    def cf_look_vector_lv(self, ic):
        ...
        t = pd.read_csv("campt.csv")
        return CartesianFixedLookVector(
            t["LookDirectionBodyFixedX"][0],
    ...
    def glas_cam_model(self, focal_length):
        ...
        return GlasGfmCamera.create_glas_from_sc_look_vector(
            sclist, focal_length=focal_length
        )
    ...
    def gfm_cam_model(self, focal_length, number_line):
        ...
        t = pd.read_csv("campt.csv")
        x = np.array(t["LookDirectionCameraX"][:]).reshape(
            (number_line, self.number_sample)
        )
        y = np.array(t["LookDirectionCameraY"][:]).reshape(
            (number_line, self.number_sample)
        )
        z = np.array(t["LookDirectionCameraZ"][:]).reshape(
            (number_line, self.number_sample)
        )
