from geocal import IterativeMorphologicalDilation, LinearGradientBadPixelDetection
...
def test_linear_gradient_bad_pixel_detection():
    ...
    if False:
        plt.imshow(original_data, cmap=plt.cm.gray, vmin=0.0, vmax=20.0)
        plt.show()
    ...
    assert (
        np.argwhere(is_bad == True)  # noqa: E712
        == [
            [0, 100],
            [100, 200],
            [300, 400],
...
def test_bad_pixel_fill():
    ...
    for i in range(-krhs, krhs + 1):
        for j in range(-kchs, kchs + 1):
            kernel[i + krhs, j + kchs] = math.exp(
                -(i * i + j * j) / (2 * sigma * sigma)
            )
    if False:
        plt.imshow(data_with_bad_pixel, cmap=plt.cm.gray, vmin=0.0, vmax=20.0)
        plt.show()
    try:
    ...
    assert (
        np.median(np.abs(original_data[10:16, 20:31] - m.filled_image[10:16, 20:31]))
        < 0.2
    )
    if False:
        plt.imshow(m.filled_image, cmap=plt.cm.gray, vmin=0.0, vmax=20.0)
        plt.show()


@pytest.mark.skip  # Don't normally run
