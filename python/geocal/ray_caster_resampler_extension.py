...
def ray_caster_project(
    ...
    nline_process = (
        int(round(nline_process / number_line_framelet)) * number_line_framelet
    )
    rcast = IgcRayCaster(
        igc, number_line_framelet, 0, -1, nintegration_step, map_info.resolution_meter
    )
    if nsub_line > 0:
        rcast.number_sub_line = nsub_line
    if nsub_sample > 0:
    ...
    with NamedTemporaryFile(dir=os.path.dirname(out_fname)) as fh:
        ...
        if dtype == np.float32 or dtype == np.float64:
            res = np.zeros(
                (map_info.number_y_pixel, map_info.number_x_pixel), dtype=np.float64
            )
            rsamp.final_rad_step_double(igc.image, rcast_data, res, scratch_count)
        else:
            res = np.zeros(
                (map_info.number_y_pixel, map_info.number_x_pixel), dtype=np.int32
            )
