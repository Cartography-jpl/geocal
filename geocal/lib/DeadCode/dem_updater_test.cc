...
BOOST_AUTO_TEST_SUITE(dem_updater)

BOOST_AUTO_TEST_CASE(basic_test)
{
  return;			// Skip this test, this is code that
				// we tried out but probably aren't
				// going to keep.
  ...
  MapInfo mi(boost::shared_ptr<CoordinateConverter>(new GeodeticConverter), 
	     110.986505555556, 
	     34.5149611111111, 111.016248575856, 34.5014740586911, 5000, 3000);
  ...
  DemUpdater u(img1, img2, r1, r2, d, mi);
  BOOST_CHECK_CLOSE(u.compare_image(781, 1277, 783.0), 460420.0, 1e-2);
  BOOST_CHECK_CLOSE(u.find_height(781, 1277), 795.034, 1e-2);
}
