...
unit_test_data = (
    os.path.abspath(os.path.dirname(__file__) + "/../../unit_test_data/") + "/"
)
...
require_mars_spice = pytest.mark.skipif(
    ...
    or not "MARS_KERNEL" in os.environ
...
require_afids_data = pytest.mark.skipif(
    not "AFIDS_VDEV_DATA" in os.environ,
    reason="need to have AFIDS data available to run.",
)

require_srtm_data = pytest.mark.skipif(
    not "ELEV_ROOT" in os.environ or not os.path.exists(os.environ["ELEV_ROOT"]),
...
require_raid = pytest.mark.skipif(True, reason="require /raid* test data to run")
...
def mars_kernel():
    ...
    if (
        SpiceHelper.have_spice()
        and "MARS_KERNEL" in os.environ
        and os.path.exists(os.environ["MARS_KERNEL"] + "/mro_kernel/mro.ker")
        and os.path.exists(os.environ["MARS_KERNEL"] + "/mex_kernel/mex.ker")
    ):
