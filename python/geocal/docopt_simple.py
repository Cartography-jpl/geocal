...
class DocOptSimple(object):
    ...
    def __init__(self, doc : str, argv : list[str] | None =None, default_help : bool=True, version : str | None=None, options_first : bool=False):
        self.args = docopt(
            doc, argv=argv, help=default_help, version=version, options_first=options_first
        )
...
class DocOptSimpleVicar(object):
    ...
    def __init__(self, argv : list[str] | None, long_name_to_vicar : dict[str, str] | None) -> None:
        if argv is None:
            argv = sys.argv
        self.argv = argv
