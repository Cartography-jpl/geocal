...
BOOST_AUTO_TEST_CASE(basic_test)
{
  ...
  try {
    d.reset(new VicarLiteDem("/Users/smyth/AlStuff/china_aoi_wgs84.hlf"));
  } catch(std::exception&) {	// Allow this to fail, since we don't
				// want to depend on the rather large
  ...
  MapInfo midem(boost::shared_ptr<CoordinateConverter>(new GeodeticConverter), 
	     110.990669578398, 34.5095662901431,
	     110.996618182458, 34.5050706060031, 1000 / 100, 1000 / 100);
  Rpc r1, r2;
  ...
  for(boost::multi_array_types::size_type i = 0; 
      i < md->elevation_data.shape()[0]; ++i)
    for(boost::multi_array_types::size_type j = 0; 
	j < md->elevation_data.shape()[1]; ++j) 
      md->elevation_data[i][j] = d->height_reference_surface(*md->map_info().ground_coordinate(j, i));
