...
def _check_plugin(cls, f, iid1, plugin, check_corner=True, corner_tol=1.0, dist_tol=-1):
    ...
    try:
        ...
        if check_corner:
            ...
            gpt1_calc = igc.ground_coordinate_approx_height(
                geocal_swig.ImageCoordinate(0, 0), gpt1.height_reference_surface
            )
            gpt2_calc = igc.ground_coordinate_approx_height(
                geocal_swig.ImageCoordinate(0, nsamp), gpt2.height_reference_surface
            )
            gpt3_calc = igc.ground_coordinate_approx_height(
                geocal_swig.ImageCoordinate(nline, nsamp), gpt3.height_reference_surface
            )
            gpt4_calc = igc.ground_coordinate_approx_height(
                geocal_swig.ImageCoordinate(nline, 0), gpt4.height_reference_surface
            )
            ...
            if dist_tol > 0:
                if (
                    geocal_swig.distance(gpt1_calc, gpt1) > dist_tol
                    or geocal_swig.distance(gpt2_calc, gpt2) > dist_tol
                    or geocal_swig.distance(gpt3_calc, gpt3) > dist_tol
                    or geocal_swig.distance(gpt4_calc, gpt4) > dist_tol
                ):
