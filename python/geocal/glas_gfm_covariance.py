...
class GlasGfmNitfTimeWrapper:
    ...
    def __getitem__(self, key):
        dt = self.cscsdb.get_raw_bytes(self.field_base + "_date", key).decode("utf-8")
        tm = self.cscsdb.get_raw_bytes(self.field_base + "_time", key).decode("utf-8")
    ...
    def __setitem__(self, key, val):
        ...
        getattr(self.cscsdb, self.field_base + "_date")[key] = dt.replace("-", "")
        # Put in HHMMSS.nnnnnnnnn format and assign
        getattr(self.cscsdb, self.field_base + "_time")[key] = pynitf.NitfLiteral(
            ("%016.9lf" % float(tm.replace("Z", "").replace(":", ""))).encode("utf-8")
        )
...
class GlasGfmNitfTime2Wrapper:
    ...
    def __getitem__(self, key):
        dt = self.cscsdb.get_raw_bytes(self.field_base + "_date", key).decode("utf-8")
        tm = self.cscsdb.get_raw_bytes(self.field_base + "_time", key).decode("utf-8")
        return Time.parse_time(f"{dt[0:4]}-{dt[4:6]}-{dt[6:8]}T00:00:00Z") + float(tm)
    ...
    def __setitem__(self, key, val):
        ...
        getattr(self.cscsdb, self.field_base + "_time")[key] = pynitf.NitfLiteral(
            ("%015.9lf" % float(val - tmidnight)).encode("utf-8")
        )
...
if have_pynitf:
    ...
    pynitf.DesCSCSDB.corr_ref_geocal_time = property(
        partial(GlasGfmNitfTimeWrapper, "corr_ref")
    )
    pynitf.DesCSCSDB.adj_parm_id_array = property(
        partial(GlasGfmNitfArrayWrapper, "adj_parm_id", "num_adj_parm")
    )
    pynitf.DesCSCSDB.errcov_c1_array = property(
        partial(GlasGfmNitfCovWrapper, "errcov_c1", "num_adj_parm")
    )
    pynitf.DesCSCSDB.post_start_geocal_time = property(
        partial(GlasGfmNitfTime2Wrapper, "post_start")
    )
    pynitf.DesCSCSDB.errcov_c2_array = property(
        partial(GlasGfmNitfCovWrapper, "errcov_c2", "num_adj_parm")
    )
    pynitf.DesCSCSDB.errcov_c2_1_array = property(
        partial(GlasGfmNitfCovWrapper, "errcov_c2_1", "num_adj_parm")
    )
...
class GlasGfmIO(object):
    ...
    def read_des(cls, cscsdb):
        if cscsdb.io_cal_ap != 0:
            warnings.warn(
                "Skipping IO section of covariance, we don't support this yet"
            )
        return cls()
...
class GlasGfmDirectCovariance(object):
    ...
    def read_des(cls, cscsdb):
        if cscsdb.direct_covariance_flag != 0:
            warnings.warn(
                "Skipping Direct Covariance section of covariance, we don't support this yet"
            )
        return cls()
...
class GlasGfmTS(object):
    ...
    def read_des(cls, cscsdb):
        if cscsdb.ts_cal_ap != 0:
            warnings.warn(
                "Skipping TS section of covariance, we don't support this yet"
            )
        return cls()
...
class GlasGfmUnmodeledError(object):
    ...
    def __init__(self, cov=None, line_spdcf_id=None, sample_spdcf_id=None):
    ...
    @classmethod
    def read_des(cls, cscsdb):
        res = cls(line_spdcf_id=cscsdb.line_spdcf, sample_spdcf_id=cscsdb.sample_spdcf)
        if cscsdb.ue_flag == 1:
            res.cov = np.empty((cscsdb.line_dimension, cscsdb.sample_dimension, 2, 2))
...
class GlasGfmCoreSet(object):
    ...
    def __init__(
        self, ref_frame_position=ECF, ref_frame_attitude=ECF, group=GlasGfmGroupList()
    ):
...
class GlasGfmGroup(object):
    ...
    def __init__(
        self,
        corr_ref_t=Time.parse_time("2000-01-01T00:00:00Z"),
    ...
    def _read_des(cls, cscsdb, i, j):
        ...
        if cscsdb.post_pl_flag[i, j]:
            warnings.warn("Skipping PL section of group, we don't support this yet")
        res = GlasGfmGroup(
            corr_ref_t=cscsdb.corr_ref_geocal_time[i, j],
        ...
        if cscsdb.post_sub_alloc[i, j]:
            ...
            if cscsdb.common_posts_cov[i, j] == 1:
                ...
                res.errcov_c2 = np.empty(
                    (res.num_post, cscsdb.num_adj_parm[i, j], cscsdb.num_adj_parm[i, j])
                )
    ...
    def _write_des(self, cscsdb, i, j):
        ...
        if self.post_sub_alloc:
            ...
            if len(self.errcov_c2.shape) == 2:
                ...
                if (
                    self.errcov_c2.shape != 3
                    or self.errcov_c2.shape[0] != self.num_post
                ):
...
class GlasGfmSpdcfCsm(GlasGfmSpdcf):
    ...
    def __str__(self):
        return (
            f"CSM four parameter SPDCF ({self.A}, {self.alpha}, {self.beta}, {self.T})"
        )
...
class GlasGfmSpdcfPiecwiseLinear(GlasGfmSpdcf):
    ...
    def __init__(
        self, max_correlation, tau_max_correlation, weight=1.0, id_number=None
    ):
        super().__init__(weight, id_number)
   ...
   {self.tau_max_correlation}"""
    ...
    def _read_des(cls, cscsdb, n, j):
        ...
        return cls(
            [cscsdb.pl_max_cor[n, j, k] for k in range(nseg)],
...
class GlasGfmSpdcfComposite(GlasGfmSpdcf):
    ...
    def _read_des(cls, cscsdb, n, j=None):
        ...
        if j is not None:
            return None
        return cls(
            [GlasGfmSpdcf._read_des(cscsdb, n, i) for i in range(cscsdb.spdcf_p[n])],
            id_number=cscsdb.spdcf_id[n],
        )
...
def _glas_gfm_covariance(iseg):
    ...
    possible = [
        ...
        if d.subheader.desid == "CSCSDB"
        and (d.des.user_subheader.numais == "ALL" or lv in d.des.user_subheader.aisdlvl)
    ]
    if len(possible) == 0:
        return None
    if len(possible) > 1:
        raise RuntimeError(
            "Found more than one possible covariance_cscsdb for image esegment level %d"
            % lv
        )
...
if have_pynitf:
    pynitf.NitfImageSegment.glas_gfm_covariance = property(
        _glas_gfm_covariance, _glas_gfm_covariance_set
    )
