...
from fixtures.require_check import require_vicarb, require_vicar, require_carto
...
def test_scinterp():
    ...
    att = Quaternion_double(
        0.06938316945407, 0.20331591888905, 0.82545247058087, -0.52199313380038
    )
    tatt = tpos
    tinterp = tpos
    ...
    npt.assert_almost_equal(res.sc_to_ci.R_component_1, att.R_component_1, 6)
    npt.assert_almost_equal(res.sc_to_ci.R_component_2, att.R_component_2, 6)
    npt.assert_almost_equal(res.sc_to_ci.R_component_3, att.R_component_3, 6)
    npt.assert_almost_equal(res.sc_to_ci.R_component_4, att.R_component_4, 6)


@require_vicarb
@require_vicar
@require_carto
def test_sc2rpc(unit_test_data):
    """Test data from devsc2rpc"""
    EciTodBurl.set_delta_ut1(0.1128609)
    od = QuaternionOrbitData(
        Time.time_acs(215077459.471879),
        EciTodBurl(3435100.496, 945571.538, -6053387.573),
        [0, 0, 0],
        Quaternion_double(0.946366, 0.0, -0.323096813, 0.0),
    )
    leapsecond_file = unit_test_data / "leapsecond.dat"
    res = sc2rpc(
        od,
        EciTodBurl.get_delta_ut1(),
        leapsecond_file,
        ImageCoordinate(1, 1),
        0.0,
...
@require_vicar
@require_carto
def test_compare_sc2rpc(unit_test_data):
    ...
    def sc2calc(ic: ImageCoordinate):
        return sc2rpc(
            od,
            EciTodBurl.get_delta_ut1(),
            leapsecond_file,
    ...
    gc_cam = od.surface_intersect(
        cam,
        FrameCoordinate(ic.line, ic.sample),
