...
class LandsatUpdate(geocal.VicarCartLabMultifileSetup):
    ...
    def __init__(self):
        ...
        lon_ref = -80
        size_tile_deg = 1;
        mi_ref = geocal.MapInfo(geocal.GeodeticConverter(), lon_ref, lat_ref + size_tile_deg, 
        ...
        for fname in glob.glob(f"{dname}/*.vic"):
            m = re.match(r'([ns])(\d+)([ew])(\d+)_gcp_15m_b8.vic', os.path.basename(fname))
            if(m):
        ...
        mi = mi_ref.subset(smin, lmin, smax - smin + mi_ref.number_x_pixel,
		           lmax - lmin + mi_ref.number_y_pixel)
        super().__init__(mi)
        for loff, soff, fname in d:
            self.file_add(loff - lmin, soff - smin, loff - lmin + mi_ref.number_y_pixel,
                          soff - smin + mi_ref.number_x_pixel, fname)
