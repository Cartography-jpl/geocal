...
BOOST_AUTO_TEST_CASE(serialization)
{
  ...
  boost::shared_ptr<RasterImage> im(new ConstantRasterImage(mi, 20));
  std::string d = serialize_write_string(im);
  if(false)
    std::cerr << d;
  boost::shared_ptr<RasterImage> imr = serialize_read_string<RasterImage>(d);
  BOOST_CHECK_EQUAL(imr->number_line(), 5);
  BOOST_CHECK_EQUAL(imr->number_sample(), 10);
  for(int i = 0; i < imr->number_line(); ++i)
    for(int j = 0; j < imr->number_sample(); ++j)
      BOOST_CHECK_EQUAL((*imr)(i, j), 20);
}
