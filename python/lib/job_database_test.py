...
def test_job_database():
    ...
    for jid in jdb.keys():
        del jdb[jid]
    jid = jdb.add_job("jobs_database_dir", ["ls"])
...
def test_handle_running_job():
    ...
    for jid in jdb.keys():
        del jdb[jid]
    jid_running = jdb.add_job("jobs_database_dir", ["ls"])
    jdb.set_status(jid_running, "running")
    jid_could_run = jdb.add_job("jobs_database_dir", ["ls"])
...
def test_handle_failed_job():
    ...
    for jid in jdb.keys():
        del jdb[jid]
    jid = jdb.add_job("jobs_database_dir", ["ls", "--bad-argument"])
    assert jdb.check_and_run()
