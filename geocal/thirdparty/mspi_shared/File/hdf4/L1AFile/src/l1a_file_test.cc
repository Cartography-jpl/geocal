...
!MODULE NAME: l1a_file_test.cc

!ABSTRACT:

   This does unit testing for the class L1AFile.
...
using std::cerr;
using MSPI::Shared::HDF4::L1AFile;
...
public:
  SimpleFieldMap() {
    ...
    init_grid(47, "470-Q", 4, "p1", "p2", "x0n","d0n");
    init_grid(42, "470-U", 4, "p1", "p2", "x45n","d45n");
  };
};
...
class SimpleFieldMap4 : public FieldMap {
public:
  SimpleFieldMap4() {
    init_grid(63, "355-I", 4, "p1", "p2", "xn","dn");
    init_grid(47, "470-Q", 3, "p1", "p2", "x0n");
  };
};
...
//-----------------------------------------------------------------------
    ...
    test_number++;
    std::string filename = "File/hdf4/L1AFile/tmp/l1a_file_test.hdf";
    SimpleFieldMap simple_field_map;
    L1AFile file1(filename, MSPI::Shared::HDF4::L1AFile::CREATE, simple_field_map);
...
//-----------------------------------------------------------------------
    ...
    if (file1.granule_id() != "l1a_file_test.hdf") {
...
//-----------------------------------------------------------------------
    ...
    test_number++;
    int max_number_subframe_expect = 35;
    int max_number_subframe = file1.max_number_subframe();
    if (max_number_subframe != max_number_subframe_expect) {
      cerr << "max_number_subframe = "<<max_number_subframe
	   << " (expected "<<max_number_subframe_expect<<")\n";
      cerr << "Unexpected result at line "<<__LINE__<<"\n";
      error = true;
    }
...
//-----------------------------------------------------------------------
    ...
    try {
      ...
      if (strcmp(e.what(), "No record for row: 197")) {
...
//-----------------------------------------------------------------------
    ...
    try {
      ...
      caught_exception = true;
      if (strcmp(e.what(), "Argument check: Frame_number != number_frame(Row_number), Frame_number = 1000, number_frame(63) = 5")) {
	cerr << "write_frame() exception = "<<e.what()<<"\n";
	cerr << "Unexpected result at line "<<__LINE__<<"\n";
	error = true;
      }
...
//-----------------------------------------------------------------------
    ...
    try {
      ...
      caught_exception = true;
      if (strcmp(e.what(), "Argument check: Frame_number != number_frame(Row_number), Frame_number = 1, number_frame(63) = 5")) {
	cerr << "write_frame() exception = "<<e.what()<<"\n";
	cerr << "Unexpected result at line "<<__LINE__<<"\n";
	error = true;
      }
...
//-----------------------------------------------------------------------
...

    {
      ...
      row_number = 63;
      frame_number = 5;
      std::vector<float> p1_expect(99);
      try {
	file1.write_frame(row_number, frame_number, p1_expect, p2_expect,
			  xn_expect, dn_expect);
      } catch (Exception& e) {
	caught_exception = true;
	if (strcmp(e.what(), "Argument check: P1.size() != number_subframe, P1.size() = 99, number_subframe = 6")) {
	  cerr << "write_frame() exception = "<<e.what()<<"\n";
	  cerr << "Unexpected result at line "<<__LINE__<<"\n";
	  error = true;
	}
...
//-----------------------------------------------------------------------
...

    {
      ...
      row_number = 63;
      frame_number = 5;
      std::vector<float> p2_expect(99);
      try {
	file1.write_frame(row_number, frame_number, p1_expect, p2_expect,
			  xn_expect, dn_expect);
      } catch (Exception& e) {
	caught_exception = true;
	if (strcmp(e.what(), "Argument check: P2.size() != number_subframe, P2.size() = 99, number_subframe = 6")) {
	  cerr << "write_frame() exception = "<<e.what()<<"\n";
	  cerr << "Unexpected result at line "<<__LINE__<<"\n";
	  error = true;
	}
...
//-----------------------------------------------------------------------
// Argument check: write_frame() with invalid number of subframes (too many).
//-----------------------------------------------------------------------
...

    {
      ...
      std::vector<float> p1_expect(99);
      std::vector<float> p2_expect(99);
      std::vector<float> xn_expect2(99);
      try {
	file1.write_frame(row_number, frame_number, p1_expect, p2_expect,
			  xn_expect2, dn_expect);
      } catch (Exception& e) {
	caught_exception = true;
	if (strcmp(e.what(), "Argument check: number_subframe > max_number_subframe(), number_subframe = 99, max_number_subframe = 35")) {
	cerr << "write_frame() exception = "<<e.what()<<"\n";
	cerr << "Unexpected result at line "<<__LINE__<<"\n";
	error = true;
	}
...
//-----------------------------------------------------------------------
// Argument check: write_frame() with number of subframe mismatch between
// xn and dn.
//-----------------------------------------------------------------------
    ...
    test_number++;
    {
      ...
      row_number = 63;
      frame_number = 5;
      std::vector<float> p1_expect(number_subframe-1);
      std::vector<float> p2_expect(number_subframe-1);
      std::vector<float> xn_expect3(number_subframe-1);
      try {
	file1.write_frame(row_number, frame_number, p1_expect, p2_expect, 
			  xn_expect3, dn_expect);
      } catch (Exception& e) {
	caught_exception = true;
	if (strcmp(e.what(), "Argument check: Dn number_subframe != Xn number_subframe, Dn number_subframe = 6, Xn number_subframe = 5")) {
	  cerr << "write_frame() exception = "<<e.what()<<"\n";
	  cerr << "Unexpected result at line "<<__LINE__<<"\n";
	  error = true;
	}
...
//-----------------------------------------------------------------------
// Argument check: write_frame() with invalid number of pixels in Dn.
//-----------------------------------------------------------------------
    ...
    frame_number = 5;
    boost::multi_array<unsigned short, 2>
      dn_expect2(boost::extents[number_subframe][25]);
    try {
      file1.write_frame(row_number, frame_number, p1_expect, p2_expect,
			xn_expect, dn_expect2);
    } catch (Exception& e) {
      caught_exception = true;
      if (strcmp(e.what(), "Argument check: Dn number_pixel != number_pixel, Dn number_pixel = 25, number_pixel = 1536")) {
	cerr << "write_frame() exception = "<<e.what()<<"\n";
	cerr << "Unexpected result at line "<<__LINE__<<"\n";
	error = true;
      }
...
//-----------------------------------------------------------------------
    ...
    test_number++;
    std::string filename2 = "File/hdf4/L1AFile/tmp/l1a_file_test2.hdf";
    {
      L1AFile file2(filename2, MSPI::Shared::HDF4::L1AFile::CREATE, simple_field_map);
...
//-----------------------------------------------------------------------
    ...
    try {
      L1AFile file2(filename2, MSPI::Shared::HDF4::L1AFile::READ, simple_field_map2);
    } catch (Exception& e) {
      caught_exception = true;
      if (strcmp(e.what(), "Swath name '470-Q', in file, does not match row name '470-Z', in field map.  Row_number: 47   Filename: File/hdf4/L1AFile/tmp/l1a_file_test2.hdf")) {
...
//-----------------------------------------------------------------------
    ...
    try {
      L1AFile file2(filename2, MSPI::Shared::HDF4::L1AFile::READ, simple_field_map3);
    } catch (Exception& e) {
      caught_exception = true;
      if (strcmp(e.what(), "Trouble with SWfieldinfo for required field name: ugh,  swath: 470-Q,  Filename: File/hdf4/L1AFile/tmp/l1a_file_test2.hdf")) {
	cerr << "L1AFile() exception = "<<e.what()<<"\n";
	cerr << "Unexpected result at line "<<__LINE__<<"\n";
	error = true;
      }
...
//-----------------------------------------------------------------------
    ...
    test_number++;
    std::string filename3 = "File/hdf4/L1AFile/tmp/l1a_file_test3.hdf";
    {
      L1AFile file3(filename3, MSPI::Shared::HDF4::L1AFile::CREATE, simple_field_map);
...
//-----------------------------------------------------------------------
    ...
    test_number++;
    L1AFile file3(filename3, MSPI::Shared::HDF4::L1AFile::READ, simple_field_map);
