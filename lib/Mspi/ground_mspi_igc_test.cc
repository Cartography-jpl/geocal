...
BOOST_AUTO_TEST_CASE(serialization)
{
  ...
  boost::shared_ptr<ImageGroundConnection> igc
    (new GroundMspiIgc(tstart, p, 45.0, -10.0, 1.7, time_tag,
		       test_data_dir() + "ground_mspi_camera_test.config", 2));
  std::string d = serialize_write_string(igc);
  if(false)
    std::cerr << d;
  boost::shared_ptr<ImageGroundConnection> igcr = 
    serialize_read_string<ImageGroundConnection>(d);

  CartesianFixedLookVector lv;
  boost::shared_ptr<CartesianFixed> p2;
  igcr->cf_look_vector(ImageCoordinate(0, 0), lv, p2);
  LnLookVector ln(lv, *p2);
  BOOST_CHECK_CLOSE(ln.view_zenith(), 1.75848329e+00 * Constant::rad_to_deg, 
		    1e-4);
  BOOST_CHECK_CLOSE(ln.view_azimuth(), 3.66389728e+00 * Constant::rad_to_deg,
