...
""")
    ...
    def add_job(
        self, working_dir, user, key, log_file, job_to_run, start_if_available=True
    ):
...
strftime('%Y-%m-%d %H:%M:%f', 'now'), 'queued', null, null, ?)""",
            (os.path.abspath(working_dir), user, key, pickle.dumps(job_to_run)),
        )
        self.db.commit()
        with open(log_file, "a") as fh:
            print(
                "%s INFO:geocal-python.job_database Added job %d to the job queue"
                % (datetime.datetime.now().isoformat(), cur.lastrowid),
                file=fh,
            )
        if start_if_available:
            subprocess.call(
                "job_database_check_and_run %s %s &" % (self.db_name, log_file),
                shell=True,
            )
    ...
    def abcd_job(
        ...
        cmd.extend(args)
        return self.add_job(
            working_dir, user, key, log_file, cmd, start_if_available=start_if_available
        )
    ...
    def keys(self):
        ...
        return [row[0] for row in self.db.execute("select job_id from job_status")]
...
"""):
            if pid_exists(r["pid"]):
                ...
                return False
            log.info(
                "Job %d had a run status, but the process %d is not on the system. Apparently this died, so marking job as 'interrupted"
                % (r["job_id"], r["pid"])
            )
