...
if version_info >= (3,0,0):
    new_instancemethod = lambda func, inst, cls: _cart_lab_multi_file.SWIG_PyInstanceMethod_New(func)
else:
...
if version_info >= (2,6,0):
    def swig_import_helper():
        ...
        fp = None
        try:
            fp, pathname, description = imp.find_module('_cart_lab_multi_file', [dirname(__file__)])
        except ImportError:
            import _cart_lab_multi_file
            return _cart_lab_multi_file
        if fp is not None:
            try:
                _mod = imp.load_module('_cart_lab_multi_file', fp, pathname, description)
            finally:
                fp.close()
            return _mod
    _cart_lab_multi_file = swig_import_helper()
...
SHARED_PTR_DISOWN = _cart_lab_multi_file.SHARED_PTR_DISOWN
...
class CartLabMultiFile(geocal_swig.raster_multifile.RasterMultifile):
    ...
    C++ includes: cart_lab_multi_file.h 
    """
    ...
    __swig_destroy__ = _cart_lab_multi_file.delete_CartLabMultiFile
CartLabMultiFile._v_directory_base = new_instancemethod(_cart_lab_multi_file.CartLabMultiFile__v_directory_base,None,CartLabMultiFile)
CartLabMultiFile_swigregister = _cart_lab_multi_file.CartLabMultiFile_swigregister
