...
class QuickBirdReflectance(InstrumentReflectance):
    ...
    def readMetaData(self, filename):
        ...
        for line in metafile:
            ...
            if line.find("absCalFactor = ") != -1:
                self.absCalFactors[index] = float(
                    line.strip().split("=")[1].split(";")[0]
                )
                continue

            if line.find("effectiveBandwidth = ") != -1:
                self.effectiveBandwidths[index] = float(
                    line.strip().split("=")[1].split(";")[0]
                )
                continue
            ...
            if line.find("meanSunEl = ") != -1:
                ...
                if isPanMetafile:
                    ...
                    self.pan_solarZenithAngle = 90.0 - self.pan_solarElevation
    ...
    def dn2TOAReflectance_factor(self, band):
        ...
        return (
            self.dn2TOARadiance_factor(band)
            * math.pow(self.pan_solarDist, 2.0)
            * math.pi
    ...
    def printMetadata(self):
        ...
        print(
            "pan solar elevation/zenith: ",
            self.pan_solarElevation,
            self.pan_solarZenithAngle,
        )
