...
def test_nitf_use00a_create(isolated_dir, unit_test_data):
    ...
    subprocess.check_call(
        [
            ...
            "VRT",
            "-q",
            str(unit_test_data / "vicar.img"),
            "use00a.vrt",
        ]
    )
    t = GdalRasterImage("use00a.vrt")
    tre = pynitf.TreUSE00A()
    ...
    t.close()
    subprocess.check_call(
        ["gdal_to_nitf", "-q", str(unit_test_data / "vicar.img"), "use00a.ntf"]
    )
    ...
    assert tre.n_seg == 0
    assert tre.max_lp_seg is None
    assert tre.sun_el == 62.5
    assert tre.sun_az == 129.6
    subprocess.check_call(["nitf_to_vicar", "-q", "use00a.ntf", "use00a.img"])
...
@require_pynitf
def test_gdal_raster(unit_test_data):
    t = GdalRasterImage(str(unit_test_data / "test_use00a.ntf"))
    tre = t.use00a
    assert tre.angle_to_north == 270
...
@require_pynitf
def test_tre_rpc(stereo_unit_test_data):
    """Test reading and writing a RPC"""
    rpc = VicarLiteRasterImage(str(stereo_unit_test_data / "10MAY21-1.img")).rpc
    ...
    ta.rpc = rpc_a
    rpc2 = ta.rpc
    npt.assert_almost_equal(rpc_a.error_bias, rpc2.error_bias, 5)
    npt.assert_almost_equal(rpc_a.error_random, rpc2.error_random, 5)
    npt.assert_almost_equal(rpc_a.height_offset, rpc2.height_offset, 3)
    npt.assert_almost_equal(rpc_a.height_scale, rpc2.height_scale, 3)
    npt.assert_almost_equal(rpc_a.latitude_offset, rpc2.latitude_offset, 3)
    npt.assert_almost_equal(rpc_a.latitude_scale, rpc2.latitude_scale, 3)
    npt.assert_almost_equal(rpc_a.longitude_offset, rpc2.longitude_offset, 3)
    npt.assert_almost_equal(rpc_a.longitude_scale, rpc2.longitude_scale, 3)
    npt.assert_almost_equal(rpc_a.line_offset, rpc2.line_offset, 3)
    npt.assert_almost_equal(rpc_a.line_scale, rpc2.line_scale, 3)
    npt.assert_almost_equal(rpc_a.sample_offset, rpc2.sample_offset, 3)
    npt.assert_almost_equal(rpc_a.sample_scale, rpc2.sample_scale, 3)
    for i in range(20):
        npt.assert_almost_equal(rpc_a.line_numerator[i], rpc2.line_numerator[i], 6)
        npt.assert_almost_equal(rpc_a.line_denominator[i], rpc2.line_denominator[i], 6)
        npt.assert_almost_equal(rpc_a.sample_numerator[i], rpc2.sample_numerator[i], 6)
        npt.assert_almost_equal(
            rpc_a.sample_denominator[i], rpc2.sample_denominator[i], 6
        )
...
def noruntest_nitf_rpc():
    tb = pynitf.TreRPC00B()
    ta = pynitf.TreRPC00A()
    assert not tb.is_rpc_a
    assert ta.is_rpc_a
    # Temporary, we need to create a RPC NTF file for unit_test
    t = GdalRasterImage(
        "/home/smyth/Local/NevadaTripletDem/10MAY21184820-P1BS-052366903050_01_P001.NTF"
    )
    rpc = t.rpc00b.rpc
    ...
    for i in range(20):
        assert not rpc.fit_line_numerator[i]
        assert not rpc.fit_sample_numerator[i]
