...
def test_old_mspi_format(isolated_dir, mspi_unit_test_data):
    """Test for reading the old MSPI format."""
    fname = mspi_unit_test_data / "old_tie_point/tie_point_211051000.dat"
    ...
    assert (
        distance(
            tp.ground_location,
            Ecr(
                -2.47279636204959126e06, -4.61999558631069399e06, 3.62513840793766454e06
            ),
        )
        < 1
    )
    assert tp.number_image_location == 3
    tpcol = TiePointCollection.read_old_mspi_format(
        mspi_unit_test_data / "old_tie_point"
    )
...
@require_serialize
def test_create_multiple_pass(geocal_test_tpcol, geocal_test_igc):
...
@require_serialize
def test_tie_point(geocal_test_igc, geocal_test_tpcol):
...
@pytest.mark.skp
def test_generate_tie_point_collection(
    geocal_test_igc, geocal_test_tpcol, geocal_test_igc_sim_error
):
    ...
    nimg = np.array(
        [
            igccol_original.image_ground_connection(i).view_angle()[0]
