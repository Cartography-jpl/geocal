...
def check_for_proc(proc):
    ...
    cmd2 = re.sub(r"\+?\n", "", proc)
    cmd2 = re.sub(r'"', '\\"', cmd2)
    res = subprocess.run('vicarb "%s"' % cmd2, shell=True, stdout=subprocess.PIPE)
    if re.search(rb"\[TAE-NOPROC\]", res.stdout):
        return False
    return True
...
def vicarb_run(cmd, print_results=False):
    ...
    if process.poll() != 0:
        raise subprocess.CalledProcessError(
            process.poll(), 'vicarb "%s"' % cmd2, output=res
        )
    return res
...
def vicarb_tae_path():
    ...
    try:
        ...
        res = subprocess.run(
            ["vicarb", "print_tae_path.pdf"], check=True, stdout=subprocess.PIPE
        )
...
require_picmtch5 = pytest.mark.skipif(
    not check_for_proc("picmtch5"), reason="need a pictmtch5 build to run"
)
