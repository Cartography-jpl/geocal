...
def cib01_data():
    ...
    return geocal_swig.VicarMultiFile(
        os.environ["CIB1_ROOT"] + "/cib01_db.int", os.environ["CIB1_ROOT"], ".img"
    )
...
def cib01_mapinfo(desired_resolution=None):
    ...
    mapinfo."""
    res = geocal_swig.serialize_read_generic(
        os.path.dirname(__file__) + "/cib01_mapinfo.xml"
    )
    if desired_resolution:
        resbase = res.resolution_meter
        res = res.scale(desired_resolution / resbase, desired_resolution / resbase)
    return res
...
def mars_fix_projection(fin, fout, band, hirise_correction=False):
    ...
    if re.search("Polar_Stereographic", p):
        p = re.sub(r'PARAMETER\["scale_factor",0\]', 'PARAMETER["scale_factor",1]', p)
        f.SetProjection(p)
    ...
    if hirise_correction and re.search("Equirectangular[_ ]MARS", p):
        m1 = re.search(r'PARAMETER\["latitude_of_origin",([\d.]+)\]', p)
        m2 = re.search(r'PARAMETER\["standard_parallel_1",([\d.]+)\]', p)
        if m1 and m2 and m1.group(1) != m2.group(1):
            print("MSG:----------------------------------------------------\n")
            print(
                "MSG: Doing HiRISE correction of swapping latitude_of_origin and standard_parallel_1 for the Equirectangular projection"
            )
            sys.stdout.flush()
            p = re.sub(
                r'PARAMETER\["latitude_of_origin",[\d.]+\]',
                'PARAMETER["latitude_of_origin",' + m2.group(1) + "]",
                p,
            )
            p = re.sub(
                r'PARAMETER\["standard_parallel_1",[\d.]+\]',
