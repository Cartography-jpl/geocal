...
def _image_coordinate_jac_parm_sparse(self, image_index, ground_point, jac,
    ...
    jac_in = self.image_coordinate_jac_parm(image_index, ground_point)
    for j in range(jac_in.shape[1]):
        jac[jac_row, jac_col + j] = jac_in[0, j] * line_scale
        jac[jac_row + 1, jac_col + j] = jac_in[1, j] * sample_scale

def _image_coordinate_jac_parm_fd_sparse(self, image_index, ground_point, jac,
                               jac_row, jac_col, parameter_step, 
                               line_scale = 1, 
                               sample_scale = 1):
    '''Variation that uses a finite difference'''
    jac_in = self.image_coordinate_jac_parm_fd(image_index, ground_point, 
                                               parameter_step)
    for j in range(jac_in.shape[1]):
        jac[jac_row, jac_col + j] = jac_in[0, j] * line_scale
        jac[jac_row + 1, jac_col + j] = jac_in[1, j] * sample_scale

IgcCollection.image_coordinate_jac_parm_sparse = _image_coordinate_jac_parm_sparse
IgcCollection.image_coordinate_jac_parm_fd_sparse = _image_coordinate_jac_parm_fd_sparse

def _igc_arr_image_coordinate_jac_parm_sparse(self, image_index, ground_point, jac,
                               jac_row, jac_col, line_scale = 1, 
                               sample_scale = 1):
    '''Update Jacobian for image coordinate with respect to change in 
    the parameters of the ground point. 
...
IgcArray.image_coordinate_jac_parm_sparse = _igc_arr_image_coordinate_jac_parm_sparse
