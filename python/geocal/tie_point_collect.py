...
class TiePointDiagnostic(object):
    ...
    def __init__(self, num_image):
        ...
        self.image_match_diagnostic = np.zeros(
            (num_image, max_diag_code + 2), dtype=int
        )
...
class TiePointCollect(object):
    ...
    def __init__(
        ...
        if self.igc_collection.number_image > 1:
            self.ri = RayIntersect3(
                self.igc_collection, max_ground_covariance=max_ground_covariance
            )
        ...
        for j in range(self.igc_collection.number_image):
            ...
            if map_info is None:
                if surface_image is None:
                    self.itoim[j] = IgcImageToImageMatch(igc1, igc2, self.image_matcher)
                else:
                    self.itoim[j] = SurfaceImageToImageMatch(
                        igc1,
                        surface_image[i],
                        igc2,
                        surface_image[j],
                ...
                self.itoim[j] = SurfaceImageToImageMatch(
                    igc1, igc2, map_info, self.image_matcher, grid_spacing
                )
        if self.ref_image is not None:
            dem = self.ref_dem
            if dem is None:
                dem = igc1.dem
            self.ref_igc = MapInfoImageGroundConnection(
                self.ref_image, dem, "Reference Image"
            )
            ...
            if surface_image is not None:
                ...
                self.itoim[j] = SurfaceImageToImageMatch(
                    igc1, self.ref_igc, mi, self.gcp_image_matcher, grid_spacing
                )
    ...
    def tie_point_grid(
        self, num_x, num_y, aoi=None, dem=None, border=100, pool=None, diagnostic=None
    ):
        ...
        if aoi:
            ...
            iplist = fd.interest_point_grid(img, num_y, num_x, border, pool=pool)
        ...
        if pool:
            res = pool.map(
                func, iplist, max(len(iplist) // multiprocessing.cpu_count(), 1)
            )
        else:
    ...
    def tie_point(self, ic1):
        ...
        for i in range(self.start_image_index, self.end_image_index):
            if i != self.base_image_index:
                ic2, lsigma, ssigma, success, diagnostic = self.itoim[i].match(ic1)
                diag[i] = diagnostic
                if success:
...
class TiePointCollectFM(object):
    ...
    def __init__(
        ...
        if not have_cv2:
            raise RuntimeError(
                "This class requires the openCV python library cv2, which is not available."
            )
        self.raster_image = [
            igc_collection.image(i) for i in range(igc_collection.number_image)
        ]
        if igc_collection.number_image > 1 and not skip_ray_intersect:
            self.ri = RayIntersect3(
                igc_collection, max_ground_covariance=max_ground_covariance
            )
        else:
            self.ri = None
        self.max_ground_covariance = max_ground_covariance
        if fdetect is not None:
            ...
            self.fdetect = cv2.xfeatures2d.SIFT_create(
                number_feature, number_octave_levels
            )
        if fdetect_ref is not None:
            self.fdetect_ref = fdetect_ref
        else:
            self.fdetect_ref = cv2.xfeatures2d.SIFT_create(
                number_ref_feature, number_octave_levels
            )
        ...
        if kp_and_desc_ref is not None:
            ...
            for ic1, ic2, idx in good:
                if idx in res:
                    ...
                    tp.image_coordinate(ind, ic1, 0.5, 0.5)
                tp.is_gcp = True
                tp.ground_location = self.ref_image.ground_coordinate(
                    ic2
                ).convert_to_cf()
                res[idx] = tp
    ...
    def tie_point_list(self, pool=None):
        ...
        kp_and_desc = [
            self.detect_and_compute(i, log) for i in range(len(self.raster_image))
        ]
        if self.ref_image:
...
def _point_list_surface_proj(ind1, ind2_or_ref_image, tpcol, igc1, igc2, ref_image):
    ...
    for i, tp in enumerate(tpcol):
        if tp.image_coordinate(ind1) is not None:
            ic1 = ref_image.coordinate(
                igc1.ground_coordinate(tp.image_coordinate(ind1))
            )
        else:
            ic1 = None
        if isinstance(ind2_or_ref_image, RasterImage):
            ...
            if tp.image_coordinate(ind2_or_ref_image) is not None:
                ic2 = ref_image.coordinate(
                    igc2.ground_coordinate(tp.image_coordinate(ind2_or_ref_image))
                )
            else:
                ic1 = None
        if ic1 is not None and ic2 is not None:
...
def _outlier_reject_ransac(
    ind1, ind2_or_ref_image, tpcol, threshold, igc1=None, igc2=None, ref_image=None
):
    """Internal function used to filter bad points between 2 images."""
    if not have_cv2:
        raise RuntimeError(
            "This function requires the openCV python library cv2, which is not available."
        )
    if igc1 is None:
        pt1, pt2, ind = _point_list(ind1, ind2_or_ref_image, tpcol)
    else:
        pt1, pt2, ind = _point_list_surface_proj(
            ind1, ind2_or_ref_image, tpcol, igc1, igc2, ref_image
        )
    if pt1.shape[0] < 1:
        return tpcol
...
def outlier_reject_ransac(
    tpcol, ref_image=None, threshold=3.0, igccol=None, diagnostic=None
):
    ...
    for i in range(nimg):
        for j in range(i + 1, nimg):
            if igccol is None:
                ...
                res = _outlier_reject_ransac(
                    i, j, res, threshold, igc1=igc1, igc2=igc2, ref_image=rimg
                )
    len2 = len(res)
    for i in range(nimg):
        if ref_image is not None:
